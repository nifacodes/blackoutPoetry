import React from 'react';
import { Grid, Button } from '@material-ui/core';
import _ from 'lodash';
import Loader from 'react-loader';
import Uniqid from 'uniqid';
import DehazeIcon from '@material-ui/icons/Dehaze';
import classNames from 'classnames';
import { createWordMap, getRandomNumberUpTo } from './src/utils';
import { Modal, Newspaper, SavedNewspaper, MobileNav, MobileLanding } from './src/components';
import { getLexperContent, getArticles } from './src/api';
import styles from './App.module.css';

const isMobile = window.innerWidth <= 600;
// const isMobile = (window.innerWidth <= 850 || window.innerWidth >= 1100);

class app extends React.Component {
    constructor(props) {
        super(props);

        this.state = {

            "totalArticles": [
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "Brendan Hesse",
                    "title": "Here's How Much You'll Save With an Apple-Refurbished iPhone XS",
                    "description": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS…",
                    "url": "https://lifehacker.com/heres-how-much-youll-save-with-an-apple-refurbished-iph-1841138323",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/roo6mjgthvj1jtb5ayr1.jpg",
                    "publishedAt": "2020-01-21T21:00:00Z",
                    "content": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS… [+2300 chars]",
                    "fullContentText": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones  through its online store  and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS Max (or or make the full jump to an iPhone 11). The XS and XS Max are barely two years old, and their tech is still plenty fast, so they’ll still be solid upgrades.\nThe real question, of course, is how much can you save on a refurbished iPhone XS or XS Max you buy directly from Apple. Exact final pricing depends on the model and configuration you go for, but all iPhone XS models appear to be $200 cheaper than buying new, while you’ll save around $250 for an iPhone XS Max. The highest prices you can expect to pay for the 512GB models of each phone (sans tax) are:\n-  512GB iPhone XS:  $999\n-  512GB iPhone XS Max:  $1,099\n\n(Note: all refurbished models sold by Apple are unlocked).\nFor reference, here’s a quick comparison of those same refurbished 512GB unlocked iPhone XS and XS Max models from popular third-party aftermarket sellers (these prices are based on the current stock at time of writing):\nSwappa\n-  iPhone XS:  $659\n-  iPhone XS Max:  $945\n\nBackmarket\n-  iPhone XS:  $799-$999\n-  iPhone XS Max:  $779\n\nGazelle\n-  iPhone XS:  $650\n-  iPhone XS Max:  $750\n\nRefurb.me\n-  iPhone XS:  $649-$999\n-  iPhone XS Max:  $749-$1099\n\nAs you can see, Apple’s prices are higher than other sellers, but buying from a third-party seller can be a gamble. Unless you’ve bought from a site or seller before, you can’t be sure the device runs as advertised. Some sellers offer warranties or trade-in programs should you wind up with a bad purchase, but it’s not a guarantee.\nConversely, all of Apple’s refurbished devices are  cleaned and tested by Apple  to ensure they meet the company’s quality standards. They’re not going to be as flawless as a brand-new device, but it can be pretty close. You’ll also get a one-year warranty on all devices and can extend coverage through AppleCare+.\nTo be clear, we’re not saying that you should only buy a refurbished iPhone XS or XS Max from Apple, but there are certain benefits Apple’s program guarantees its customers that can make buying one of their refurbished devices less risky, even with the higher prices. If you can find a better deal you trust elsewhere, then go for it. If not, Apple’s store is a decent option if you’re in the market for a slightly cheaper iPhone XS.",
                    "id": "k69plbdg"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "Abu Zafar",
                    "title": "How to Take a Screenshot on iPhone X, XS and 11",
                    "description": "If you’ve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhon…",
                    "url": "https://lifehacker.com/how-to-take-a-screenshot-on-iphone-x-xs-and-11-1841064538",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/jz1kqcyhtcozukg1wike.jpg",
                    "publishedAt": "2020-01-21T14:30:00Z",
                    "content": "If youve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhone… [+330 chars]",
                    "fullContentText": "If you’ve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhone X redesign in 2017, the button combination has changed. In the short video above, I show you two ways to take a screenshot on a modern iPhone.\nQuick Fix is a video series where I tackle your most commonly asked tech questions in 90 seconds or less. If you have a question you want me to answer, leave a comment below or email me.",
                    "id": "k69plbdh"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "You Can't Stream 'Star Trek: Picard' on Linux Without This Workaround",
                    "description": "It’s 2020, and you think that it would be easy to stream any content you want on any device you want—especially that brand-new Picard series on CBS All Access. If you’re an ubergeek who loves both Star Trek and Linux, you’re out of luck. Thanks to the joys of…",
                    "url": "https://lifehacker.com/you-cant-stream-star-trek-picard-on-linux-without-this-1841327097",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/v4nesxfyduqgoy7qpdct.png",
                    "publishedAt": "2020-01-29T19:00:00Z",
                    "content": "Its 2020, and you think that it would be easy to stream any content you want on any device you wantespecially that brand-new Picard series on CBS All Access. If youre an ubergeek who loves both Star Trek and Linux, youre out of luck. Thanks to the joys of dig… [+2005 chars]",
                    "fullContentText": "It’s 2020, and you think that it would be easy to stream any content you want on any device you want—especially that brand-new  Picard  series on CBS All Access. If you’re an ubergeek who loves both  Star Trek  and Linux, you’re out of luck. Thanks to the joys of digital rights management, or DRM, you’re unable to use CBS’ All Access service to stream Picard on Linux.\nIn a cruel twist, not only can you not stream the show, but you  can  stream all the bullshit that comes wrapped around it. As Ars Technica’s Jim Salter  describes  :\n“Moving back to one of my own PCs, I was quickly able to confirm the issue: trailers autoplay properly, and even the ads work—but the actual content won’t play on a Linux desktop PC on any browser including Google Chrome. Diving into the Chrome Web Console, we can see HTTP 400 (Bad Request) errors when the browser attempts to fetch a license from CBS’  Widevine  back end.”\nThe more tech-savvy among you are probably assuming that you can just change your browser’s user agent to fool CBS All Access into thinking you’re using a supported platform. Not so. (Salter tried that, too.) For whatever reason, Widevine and CBS All Access don’t play nice with Linux, even though other streaming services that use Widevine have no issues.\nThe only workaround to this vexing problem, aside from watching CBS All Access on a different device entirely, is to cancel your regular CBS All Access subscription and instead subscribe to the service via  Amazon Prime Video  . Yes, you’ll need to be an existing Amazon Prime member for this to work (in the U.S., at least), which will already set you back $120 annually. After that, adding CBS All Access costs the same as if you were subscribing standalone: $6/mo for limited commercials or $10/mo for commercial-free streaming.\nThat is, however, slightly more than you’d pay if you signed up for an annual plan  via CBS itself  ($60/year for limited commercials and and $100/year for no commercials). Amazon doesn’t offer these options, so this route is best if you’re only planning to binge a show or two and then cancel your service. If you’re looking to stick with CBS All Access for the long haul, you might just want to try not watching it on Linux; a tablet, phone, or app on your Smart TV will save you more cash.",
                    "id": "k69plbdi"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "Josh Ocampo",
                    "title": "Why You Shouldn't Let Celebrities Text You Via Community",
                    "description": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on socia…",
                    "url": "https://lifehacker.com/why-you-shouldnt-let-celebrities-text-you-via-community-1841259489",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/sgh5mxng1qcls0vqmmp6.jpg",
                    "publishedAt": "2020-01-27T18:30:00Z",
                    "content": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on socia… [+1776 chars]",
                    "fullContentText": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on social media, they’re likely about to target you even more closely.\nRecently, if you’ve ever stumbled across a tweet or Instagram in which a celebrity posts their “phone number” so that they can interact with and text you directly, it’s not what you think—instead of entering into a personal chat with a celebrity, you’ve  actually  subscribed to a newsletter of automated updates that’ll be sent by text.\nBack in October, reporter Taylor Lorenz described how it works in detail for the  New York Times  . If you “text” a celebrity using this number, you’re likely signing up for a service run by a tech-startup called  Community  ; they help manage direct messaging with large audiences on behalf of celebrities and other well-known figures, as a way of “monetizing” their social network.\nOnce you opt-in by way of text, what kind of updates might you receive? Well, it depends. Sometimes, it’s just a random string of photos and texts, as Lorenz received from actor Ashton Kutcher. I signed up for  Pod Save America’s  Jon Favreau’s updates and just received a bunch of updates about their tour.\nIf you’re really, really lucky, you  could  a text back from the celebrity in question—but don’t count on it. Given the number of messages they likely receive, it would seem like a huge effort just to respond to every text. The vast majority of the time, you’re likely only at the receiving end of these automated updates.\nIf you’re a particular fan of a celebrity, maybe signing up isn’t such a bad idea. But we’ll do you a favor and tell you now that it probably isn’t worth it. The odds are incredibly low that you’ll receive a text that isn’t just an automated update, and while  Community ensures  that they will not sell your personal details to other third parties, well,  we’ve all heard that before  , haven’t we?",
                    "id": "k69plbdj"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "Help! I Bumped My PC and Now It Won't Turn On",
                    "description": "Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could…",
                    "url": "https://lifehacker.com/help-i-bumped-my-pc-and-now-it-wont-turn-on-1841360774",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/o2oyoxjrp7m2htuheipk.jpg",
                    "publishedAt": "2020-01-31T14:00:00Z",
                    "content": "Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could… [+5747 chars]",
                    "fullContentText": "Tech 911  Do you have a tech question keeping you up at night? We'd love to answer it! Email david.murphy@lifehacker.com with \"Tech 911\" in the subject line.  Prev  Next Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could end it all.\nYour desktop PC is a bit more protected, in theory, since it probably lives in a bulky case. That doesn’t mean it’s invulnerable. Though we weren’t sent this question by a Lifehacker reader, as is custom for the  Tech 911  column, I stumbled across this call for help on Reddit and couldn’t resist highlighting it in this this week’s edition. It’s just that good.\nRedditor  u/dyingquickly  describes  :\n“I lightly bumped my computer it shot off and makes no attempt to boot when I hit the power button. I know my PSU is working because I have a tester. I’ve triple checked all my connections. Does anyone know what the problem might be? The light of the on button doesn’t even appear, it’s as if the button isn’t working but i don’t know what the problem is.”\nGenerally speaking, when your computer zaps off like that, it’s a pretty good indication that you’re going to have to do some troubleshooting. If you’re lucky, you’re only dealing with a random power outage or loose cable—possibly even a button on your surge protector that you brushed up against. You should be able to correct the problem and turn your system back on again, suffer a quick disk scan, and resume whatever it is you were doing. (Here’s hoping your work or  browser tabs  autosaved.)\nWhen your system  doesn’t  turn on is when this process really gets fun.\n\nCHECK FOR LOOSE CABLES\nSuppose you bang into your desktop PC with your chair, your foot, or your stupid but loveable pet runs into it. We’ll classify this as a “hard” bump, not a gentle dusting. If your desktop PC won’t turn back on—as in, it won’t even POST—my first assumption is a crucial cable has fallen out of a connector  just enough  to cause an issue.\nThis is going to be annoying, but I recommend popping open your desktop PC and making sure everything looks right. If some cable isn’t connected in some awkward way, that might be the culprit—though your motherboard’s primary power cables  should  have locking mechanisms that make this impossible.  I wouldn’t rule anything out  . In fact, I’d start with them: the fat 24-pin cable and the smaller 4/8-pin cable.\n\nSTART DISCONNECTING AND RECONNECTING CRITICAL CABLES\nIf a loose cable isn’t immediately apparent, you might want to take the more annoying step of unplugging everything that’s plugged in and reconnecting it. I’d start with more critical cables first—those going into your motherboard, graphics card, and modular power supply (if applicable). You can try your SATA cables, too, as well as the power connections for your hard drive(s). (Though your motherboard should still be able to POST without a hard drive connected, let’s not take any chances.)\nYou’ll also want to check your system’s tiny front-panel connectors. While a loose front-panel cable would explain why you can’t turn your system back on, you’ll also likely be able to surmise this is the issue if your motherboard is otherwise powered on (via any lights you see). If your motherboard has its own power button you can use to start your system, and pressing that makes everything work correctly, then you’re likely dealing with some issue with your front-panel cables.\nAlso, make sure you’re checking the rear of your system. There are plenty of cables back there—the power cable, especially—so it’s possible that something got jiggered out of place in such a way as to prevent your computer from turning on. Again, this might feel like try-anything troubleshooting, but I’m willing to bet that the answer to this issue is going to be some random cable you wouldn’t have normally thought about in your troubleshooting. Heck, you can even try resetting the CMOS battery; I doubt it’s the culprit, but it’s worth exploring if your other ideas don’t pan out.\nIt’s also possible that you bumped something out of place in your case that is now causing some kind of short-circuit with your motherboard. That could be a cable; that could be a screw that you accidentally left in your system when you were building it. However, short of eyeballing the problem, taking your system apart and rebuilding it is likely going to be the best way to address this. Given how long that might take to do, I’d save it as a final option.\nAnd if you go this route, be  methodical  . Strip your system to the bare components. Take the CPU and cooler off your motherboard, remove the RAM, and unplug everything (to name a few steps). Once you’re staring at a bunch of components, plug everything back in as if you’re  building a computer for the first time  . Connect the basics—your motherboard, your RAM, and your video card (if applicable)—and try powering on your system. Can you make it into the BIOS? If so, power down your PC and start adding your other hardware one at a time, until you can deduce what, if anything, is causing issues.\n\nTRY FIGURING OUT WHICH COMPONENT HAS FAILED\nAnd if you still can’t power on your PC, you might want to phone a friend. Slap your hard drive into their system or external enclosure to confirm your data is safe (and back it up, if need be)? See if you can borrow an extra power supply to use with your core components. Do they have any extra RAM you could try? Will your video card work on their system?\nAt this point, you’re likely trying to pinpoint faulty hardware, since odds are getting higher that you actually did some kind of damage to something inside your system. Hopefully you don’t have to reach this point; I’ve been there, and it’s not a fun bit of detective work.\nDo you have a tech question keeping you up at night? Tired of troubleshooting your Windows or Mac? Looking for advice on apps, browser extensions, or utilities to accomplish a particular task? Let us know! Tell us in the comments below or email  .",
                    "id": "k69plbdk"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "Change These Facebook Settings to Protect Your Photos From Facial Recognition Software",
                    "description": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesn’t make this weekend’s report from The New York Times any less dam…",
                    "url": "https://lifehacker.com/change-these-facebook-settings-to-protect-your-photos-f-1841131425",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/kgodep1gxupxb0lqj9je.png",
                    "publishedAt": "2020-01-21T20:00:00Z",
                    "content": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesnt make this weekends report from The New York Times any less damni… [+4316 chars]",
                    "fullContentText": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesn’t make this weekend’s report from  The New York Times  any less damning, it’s a great reminder about how important it is to  really  go through the settings for your various social networks and limit how your content is, or can be, accessed by anyone.\nI won’t get too deep into the  Times’  report; it’s worth reading on its own, since it involves a company (Clearview AI) scraping more than three billion images from millions of websites, including Facebook, and creating a facial-recognition app that does a pretty solid job of identifying people using images from this massive database.\nEven though Clearview’s scraping techniques technically violate the terms of service on a number of websites, that hasn’t stopped the company from acquiring images  en masse.  And it keeps whatever it finds, which means that turning all your online data private isn’t going to help if Clearview has already scanned and grabbed your photos.\nStill, something is better than nothing. On  Facebook  , likely the largest stash of your images, you’re going to want to visit  Settings >  Privacy  and look for the option described:  “Do you want search engines outside of Facebook to link to your profile?”\nTurn that off, and Clearview won’t be able to grab your images. That’s not the setting I would have expected to use, I confess, which makes me want to go through all of my social networks and rethink how the information I share with them flows out to the greater web.\n\nLOCK DOWN YOUR FACEBOOK EVEN MORE WITH THESE SETTINGS\nSince we’re already here, it’s worth spending a few minutes wading through Facebook’s settings and making sure as much of your content is set to friends-only as possible. That includes changing  “Who can see your future posts”  to “friends,” using the  “Limit Past Posts”  option to change everything you’ve previously posted to friends-only, and making sure that only you can see your friends list—to prevent any potential scraping and linking that some third-party might attempt. Similarly, make sure only your friends (or friends of friends) can look you up via your email address or phone number. (You never know!)\nYou should then visit the  “  Timeline and Tagging  ”  settings page and make a few more changes. That includes only allowing friends to see what other people post on your timeline, as well as posts you’re tagged in. And because I’m a bit sensitive about all the crap people tag me in on Facebook, I’d turn on the “Review” options, too. That won’t help your account from being scraped, but it’s a great way to exert more control over your timeline.\nFinally, even though it also doesn’t prevent companies from scraping your account, pull up the  “  Public posts  ”  section of Facebook’s settings page and limit who is allowed to follow you (if you desire). You should also restrict who can comment or like your public information, like posts or other details about your life you share openly on the service.\n\nONCE I FIX FACEBOOK, THEN WHAT?\nHere’s the annoying part. Were I you, I’d take an afternoon or evening and write out all the different places I typically share snippets of my life online. For most, maybe that’s probably a handful of social services: Facebook, Instagram, Twitter, YouTube, Flickr, et cetera.\nOnce you’ve created your list, I’d dig deep into the settings of each service and see what options you have, if any, for limiting the availability of your content. This might run contrary to how you use the service—if you’re trying to gain lots of Instagram followers, for example, locking your profile to “private” and requiring potential followers to request access might slow your attempts to become the next big Insta-star. However, it should also prevent anyone with a crafty scraping utility to mass-download your photos (and associate them with you, either through some fancy facial-recognition tech, or by linking them to your account).\nWill you be able to fully prevent someone from finding a way to build the next big database of searchable images? Probably not. But the most important lesson we can take from situations like these—Clearview AI’s...well...existence—is that you should understand the nuances of the services you share your information to, and you should always use your settings to restrict access to your data in whatever way you feel most comfortable. Were I you, I’d err on the side of “let trusted people see what I do instead of anyone on the internet,” but that’s just me.",
                    "id": "k69plbdl"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "I Lost Nine Years of Photos by Locking Myself Out of My Google Account",
                    "description": "I normally use this weekly column to answer people’s technology-themed questions. This week, I’m taking a slight departure, because I think sharing a reader’s story is important—even though there’s not really much I, nor Google, can do in the case of her lock…",
                    "url": "https://lifehacker.com/i-lost-nine-years-of-photos-by-locking-myself-out-of-my-1841178170",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/qsqfi9lclqdlo3adj7ih.jpg",
                    "publishedAt": "2020-01-24T14:00:00Z",
                    "content": "I normally use this weekly column to answer peoples technology-themed questions. This week, Im taking a slight departure, because I think sharing a readers story is importanteven though theres not really much I, nor Google, can do in the case of her locked Go… [+6981 chars]",
                    "fullContentText": "Tech 911  Do you have a tech question keeping you up at night? We'd love to answer it! Email david.murphy@lifehacker.com with \"Tech 911\" in the subject line.  Next I normally use  this weekly column  to answer people’s technology-themed questions. This week, I’m taking a slight departure, because I think sharing a reader’s story is important—even though there’s not really much I, nor Google, can do in the case of her locked Google account. There’s a lot we can learn from her example, and a few items you should check to make sure this frustrating issue never happens to you.\nLifehacker reader  Cathryn  writes:\nI stupidly forgot my Google account password back in November 2019, I had only just changed it. I asked them to email to text me a verification code, like I’d done so many times before, however I stupidly added the 2 step verification on my account.\nThe problem was I inputted an incorrect recovery email address, I missed 2 digits off my email. Therefore the 2 step verification wouldn’t work. Google locked my account, I also broke my mobile phone screen days later that I ended up buying a new handset but the same mobile phone number.\n[...]\nI only get asked 1 security question “what was your first phone number” I picked that question myself in the security settings a long time ago. Then I get asked to give them an email they can contact me on, then a few days later I receive an email stating they don’t have enough evidence so unfortunately they can’t open my account.\nI never get asked any questions such as when was your account created. I’ve tried recovery options from my laptop, kindle ect that used the same gmail account plus it’s all from my home IP address.\nI get it’s my fault for initially forgetting my password, but google security settings are flawed as when I put a recovery email they should send an automatic email confirmation to make sure it’s correct due to human error. I’ve lost lots of important document and 9 years of Google photos with some pictures of loved ones that I’ll never get back.\nI present this reader story as a cautionary tale. First, whenever you’re dealing with two-factor authentication or any kind of account-recovery options—whether you’re inputting your email or phone number to set it up, or you’re copying down precious backup codes—it’s critical that you triple-check this information. That’s not because there’s no way to change it; you can obviously change details like your email address or phone number if you have access to your account. However, you’re less likely to this because most people assume that everything is on the up and up once they’ve set up their accounts. And if it isn’t, and you left a typo in a critical account recovery option, you’re going to have a mess of a time when you try to restore access.\nGoogle makes this entire situation particularly frustrating because they, a giant search company that practically prints money, have no desire to set up a customer service center that Google users can call whenever they have issues with Google’s services. Regular (free) Google users get a  Help Center  and some community forums, but that’s it.\nAnd while I’m a little snarky about this, I do see Google’s point of view. First off, most people aren’t paying Google a dime for the services they’re using. Yes, Google is doing everything it can to extract data from you so it can better target you with relevant advertising, but most people aren’t coughing up cash for the privilege of using Gmail, for example. You might pay Google a little per month  for some extra storage  , but that’s about it. Why dedicate a not-so-insignificant amount of resources to a service center for free services?\nSecond, and more importantly, Google’s lack of one-to-one help for issues like account recovery is actually a security measure in itself. Consider the alternative: If someone sort-of knew your email address, phone number, and/or mailing address, in combination with some information they’ve pilfered from one of the many data breaches out there, would you want them to be able to convince a Google customer service representative that they are actually you?\nGiving everyone the same, generic account recovery process, and letting nobody have access to any special favors or extra support, makes it harder to get into a locked account when you have unique issues, sure. It also makes it harder for others to get into  your  account. I’m not saying I’m a fan of this trade-off—because real people  do  have complex issues that need special help—but it does help limit an unscrupulous person’s ability to social engineering their way into your Google account.\nThis puts the burden on you to make sure that all of your recovery mechanisms for your Google account are accurate. Even if you’ve already set them up and are feeling pretty good about it, I recommend taking a quick few minutes to double-check. It never hurts, because you don’t want to deal with the alternative of not being able to access your account in the event of some major mess or security breach.\nTo get started, visit the  Security  section of your Google account settings. Scroll down a bit to the “Ways we can verify it’s you” list of questions and make sure your phone number, recovery email address, and security question are all accurate.\nNext, scroll up a smidge and click on  “  2-Step Verification  .”  You’ll have to enter your password again. On the subsequent screen, make sure that you’ve turned  on  Google’s two-step verification to better protect your account. Assuming you have, scroll down a bit and click on “Show Codes” under the “Backup codes” section.\nCopy. These. Codes. Down.  To paraphrase Gandalf, keep them secret and safe; you’ll use them to log back into your account if, for whatever reason, you can’t access the device(s) you’ve set up for two-step verification.\nFinally, go visit the  Personal info  section of your Google account settings and double-check that your actual contact information is correct: email(s) and phone number(s). While you’re here, you might as well also make sure you’ve entered your correct birthday, too.\nAnd, of course, the best way to make sure you never forget your passwords to anything is to use  a strong password manager  . Even if it costs you a few bucks a month, it’s worth every penny. Not only does this make it easy to keep track of your passwords, but you can use stronger, more secure passwords that are  very  difficult to guess or brute-force. Also, a great password manager will let you know whenever your saved accounts are compromised in a data breach (which should be less of an issue if you’re using unique passwords for each service, but people can get a little lazy about this).\nThat’s my advice to everyone, and it applies to more than just Google. If a service is important to you, make sure you know the process of how you’ll get back into your account if you’re ever locked out. Then, check your data to make sure any errant typos won’t come back to haunt you, and make sure you’ve saved any backup codes or any other information you need in order to regain access to a lost or locked account. You’ll be glad you did now, if (or when) you have to deal with this later.\nDo you have a tech question keeping you up at night? Tired of troubleshooting your Windows or Mac? Looking for advice on apps, browser extensions, or utilities to accomplish a particular task? Let us know! Tell us in the comments below or email  .",
                    "id": "k69plbdm"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Theinventory.com"
                    },
                    "author": "Tercius, Ana Suarez, and Corey Foster on Kinja Deals, shared by Ana Suarez to Lifehacker",
                    "title": "Monday's Best Deals: Sony ANC Headphones, Tile Gold Box, Witcher 3: Wild Hunt, and More",
                    "description": "A windshield wiper sale, 2TB Seagate Firecuda, foam roller, and a supplement Gold Box lead of Monday’s best deals from around the web. Read more...",
                    "url": "https://kinjadeals.theinventory.com/mondays-best-deals-sony-anc-headphones-tile-gold-box-1841258693",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/fkz6rbhgodm1ezkghyji.jpg",
                    "publishedAt": "2020-01-27T15:30:00Z",
                    "content": "A windshield wiper sale, 2TB Seagate Firecuda, foam roller, and a supplement Gold Box lead of Mondays best deals from around the web.\r\nBookmark Kinja Deals and follow us on Twitter to never miss a deal.\r\neBay is offering one of the best deals on the best nois… [+14280 chars]",
                    "fullContentText": {},
                    "id": "k69plbdn"
                },
                {
                    "source": {
                        "id": "engadget",
                        "name": "Engadget"
                    },
                    "author": "Richard Lawler",
                    "title": "The Morning After: Meet Cadillac's tech-heavy new Escalade",
                    "description": "Hey, good morning! You look fabulous. Good morning! Since there's nothing at all wonky to report with Election 2020 results and incredibly flawed apps that were shoddily included in the primary process, let me point you to your next 4K demo reel. It's from 18…",
                    "url": "https://www.engadget.com/2020/02/05/2021-escalade-morning-after/",
                    "urlToImage": "https://o.aolcdn.com/images/dims?thumbnail=1200%2C630&quality=80&image_uri=https%3A%2F%2Fo.aolcdn.com%2Fimages%2Fdims%3Fresize%3D2000%252C2000%252Cshrink%26image_uri%3Dhttps%253A%252F%252Fs.yimg.com%252Fos%252Fcreatr-uploaded-images%252F2020-02%252F73cf1810-4810-11ea-bfbb-f06a233254c6%26client%3Da1acac3e1b3290917d92%26signature%3D3fa8c987b941255de22db64185ddf92abbe04f9b&client=amp-blogside-v2&signature=303b1045fccfa43bdb58bee9ac63b38f8b8e5057",
                    "publishedAt": "2020-02-05T12:30:00Z",
                    "content": "Games are capped at 1080p and 60 fps.NVIDIA's GeForce Now cloud gaming service is live\r\nAfter seven years of tweaking its delivery systems and gathering beta feedback, NVIDIA has finally unveiled the consumer version of its game-streaming ecosystem. GeForce N… [+3181 chars]",
                    "fullContentText": "Games are capped at 1080p and 60 fps.  NVIDIA's GeForce Now cloud gaming service is live\nAfter seven years of tweaking its delivery systems and gathering beta feedback, NVIDIA has  finally unveiled the consumer version of its game-streaming ecosystem  . GeForce Now streams games of all sizes to PC, Mac, Android and the NVIDIA Shield, and it works with players' existing libraries on Steam, the Epic Games Store and all other digital platforms.\nA free option allows folks to connect to NVIDIA's servers and play for one hour at a time. After an hour, these players will need to reconnect and potentially wait for a spot to open up on the servers. In the paid tier, players go to the front of the line when logging on, they'll stay online for up to six hours and get access to RTX hardware.\nAnd release dates for some highly anticipated shows.  Disney+ has 28.6 million subscribers so far\nIn Disney's first earnings report since the launch of Disney+, the company announced  how many people have signed up for its streaming service  . As of Monday, the number is 28.6 million, which CEO Bob Iger said he doesn't expect to change significantly until this fall. That's when October's  The Mandalorian  season two arrives, as well as MCU-spinoff shows  Falcon and the Winter Soldier  (August) and  Wandavision  (December).\nIt's OK, though, they apologized.  Google Photos bug sent private videos to strangers\nIf you used Google Takeout to back up your Photos files from November 21st to 25th, 2019, a stranger out there may have a copy of your videos. During that period, the email reads, \"some videos in Google Photos were incorrectly exported to unrelated users' archives.\" That means  someone out there may have a copy of your (hopefully wholesome) videos  , and you may have a copy of someone else's, as well.\nDon't forget the augmented reality navigation system.  2021 Cadillac Escalade packs 38 inches of curved OLED screens and Super Cruise\nCadillac is introducing a new Escalade later this year, and inside you'll find three curved OLED screens in the dashboard. There's a 7.2-inch touch panel to the driver's left, a 14.2-inch screen info cluster behind the steering wheel and the 16.9-inch infotainment system. With \"perfect\" black levels and bright colors, the screens don't need hoods over them to block out the light.\nThe luxury SUV line also packs five USB ports (three of them are USB-C), wireless charging, an optional 36-speaker audio system that uses mics to let front and rear passengers communicate, Super Cruise driving assist that can now change lanes by itself, NFC and electric motors that help the doors close quietly.  Check out all the details and get a closer look here  .\nThe show is scheduled to start February 24th.  LG pulls out of Mobile World Congress over coronavirus concerns\nAs concerns over a growing coronavirus outbreak continue to mount, LG has decided that the Barcelona-based trade show  simply isn't worth the risk  . Instead, LG will hold separate events in the \"near future\" to reveal its batch of 2020 smartphones, which is expected to include a sequel to last year's ambitious, underwhelming G8 ThinQ.\n\nBUT WAIT, THERE'S MORE...\n-  Iowa Democrats say sloppy app code delayed caucus results\n-  Sony sets prices for (most of) its 2020 4K LED TVs\n-  'Altered Carbon' season two teaser shows Anthony Mackie as the 'new' hero\n-  Now T-Mobile and Sprint can verify calls across their networks to prevent robocalls\n-  KFC slips ads into Spotify Premium through artist profiles\n-  Dyson patents a wearable air purifier that doubles as headphones\n-  SpaceX asks permission to take Starship on a high-altitude test flight\n-  Rockstar Games co-founder Dan Houser is leaving the company\n\nThe Morning After is a new daily newsletter from Engadget designed to help you fight off FOMO. Who knows what you'll miss if you don't  Subscribe  .\nCraving even more?  Like us on Facebook  or  Follow us on Twitter  .\nHave a suggestion on how we can improve The Morning After?  Send us a note  .\nIn this article:  av  ,  business  ,  design  ,  entertainment  ,  gadgetry  ,  gadgets  ,  gear  ,  internet  ,  mobile  ,  space  ,  themorningafter  ,  transportation  All products recommended by Engadget are selected by our editorial team, independent of our parent company. Some of our stories include affiliate links. If you buy something through one of these links, we may earn an affiliate commission.  Comments",
                    "id": "k69plbdo"
                },
                {
                    "source": {
                        "id": "engadget",
                        "name": "Engadget"
                    },
                    "author": "Rachel England",
                    "title": "Yubico is making it easier for businesses to buy its YubiKeys",
                    "description": "A growing number of companies are looking at hardware authentication security keys as a trusted and convenient way to protect sensitive corporate data. Indeed, Google has recently launched an open source project to help advance the uptake of this technology. …",
                    "url": "https://www.engadget.com/2020/02/04/yubico-business-yubikey-enterprise-security-key/",
                    "urlToImage": "https://o.aolcdn.com/images/dims?thumbnail=1200%2C630&quality=80&image_uri=https%3A%2F%2Fo.aolcdn.com%2Fimages%2Fdims%3Fcrop%3D1000%252C656%252C0%252C147%26quality%3D85%26format%3Djpg%26resize%3D1600%252C1050%26image_uri%3Dhttps%253A%252F%252Fs.yimg.com%252Fos%252Fcreatr-uploaded-images%252F2019-06%252Fdffe5260-8e13-11e9-b63f-ce5f582bb97a%26client%3Da1acac3e1b3290917d92%26signature%3De8a7567296fdb4cf66936f0c2064ac2f77b7c80d&client=amp-blogside-v2&signature=b0997fd1d0f76bb41ba86eca944320c591707ef4",
                    "publishedAt": "2020-02-04T12:00:00Z",
                    "content": "The company is initially offering two services: YubiEnterprise Subscription (which is available now), and YubiEnterprise Delivery (which is slated for release in spring). YubiEnterprise Subscription is an annual per-user pricing model that allows companies to… [+1265 chars]",
                    "fullContentText": "The company is initially offering two services: YubiEnterprise Subscription (which is available now), and YubiEnterprise Delivery (which is slated for release in spring). YubiEnterprise Subscription is an annual per-user pricing model that allows companies to subscribe to a specific service tier based on how many users they need to support. The idea is that by moving away from a per-key purchasing model, organizations can better predict future spending and upgrades. This one is designed for businesses with 750 YubiKey users or more.\nThe second offering, YubiEnterprise Delivery, takes care of the actual logistics of key distribution. Via a dedicated Yubico portal (or software that can be integrated directly into existing IT software using public APIs) IT administrators can keep a track of who needs access to which keys, and the service will manage inventory, ship keys to the relevant people, monitor delivery and manage returns or replacements.\nYubico is one of the biggest players in the security key business -- and its tech is widespread across  Windows  and  iOS  devices -- so it's not a surprise that it's the first one to make this kind of offering. And it's much-needed -- businesses overall are gradually waking up to the benefits of security keys, but until now the logistical challenges in implementing them means they've largely been the sole preserve of smaller companies. Yubico's enterprise service opens the tech up to much larger companies, which will no doubt help accelerate its adoption overall.\nIn this article:  2FA  ,  business  ,  design  ,  enterprise  ,  gear  ,  hardware authentication  ,  personal computing  ,  personalcomputing  ,  security  ,  security key  ,  service  ,  Yubico  ,  Yubikey\n\nAll products recommended by Engadget are selected by our editorial team, independent of our parent company. Some of our stories include affiliate links. If you buy something through one of these links, we may earn an affiliate commission.\n\nComments\n\n47  Shares\n\nShare\n\nShare",
                    "id": "k69plbdp"
                }
            ],
            "savedArticles": {},
            "entireCurrentArticleOF": {
                "source": {
                    "id": null,
                    "name": "Lifehacker.com"
                },
                "author": "Brendan Hesse",
                "title": "Here's How Much You'll Save With an Apple-Refurbished iPhone XS",
                "description": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS…",
                "url": "https://lifehacker.com/heres-how-much-youll-save-with-an-apple-refurbished-iph-1841138323",
                "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/roo6mjgthvj1jtb5ayr1.jpg",
                "publishedAt": "2020-01-21T21:00:00Z",
                "content": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS… [+2300 chars]",
                "fullContentText": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones  through its online store  and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS Max (or or make the full jump to an iPhone 11). The XS and XS Max are barely two years old, and their tech is still plenty fast, so they’ll still be solid upgrades.\nThe real question, of course, is how much can you save on a refurbished iPhone XS or XS Max you buy directly from Apple. Exact final pricing depends on the model and configuration you go for, but all iPhone XS models appear to be $200 cheaper than buying new, while you’ll save around $250 for an iPhone XS Max. The highest prices you can expect to pay for the 512GB models of each phone (sans tax) are:\n-  512GB iPhone XS:  $999\n-  512GB iPhone XS Max:  $1,099\n\n(Note: all refurbished models sold by Apple are unlocked).\nFor reference, here’s a quick comparison of those same refurbished 512GB unlocked iPhone XS and XS Max models from popular third-party aftermarket sellers (these prices are based on the current stock at time of writing):\nSwappa\n-  iPhone XS:  $659\n-  iPhone XS Max:  $945\n\nBackmarket\n-  iPhone XS:  $799-$999\n-  iPhone XS Max:  $779\n\nGazelle\n-  iPhone XS:  $650\n-  iPhone XS Max:  $750\n\nRefurb.me\n-  iPhone XS:  $649-$999\n-  iPhone XS Max:  $749-$1099\n\nAs you can see, Apple’s prices are higher than other sellers, but buying from a third-party seller can be a gamble. Unless you’ve bought from a site or seller before, you can’t be sure the device runs as advertised. Some sellers offer warranties or trade-in programs should you wind up with a bad purchase, but it’s not a guarantee.\nConversely, all of Apple’s refurbished devices are  cleaned and tested by Apple  to ensure they meet the company’s quality standards. They’re not going to be as flawless as a brand-new device, but it can be pretty close. You’ll also get a one-year warranty on all devices and can extend coverage through AppleCare+.\nTo be clear, we’re not saying that you should only buy a refurbished iPhone XS or XS Max from Apple, but there are certain benefits Apple’s program guarantees its customers that can make buying one of their refurbished devices less risky, even with the higher prices. If you can find a better deal you trust elsewhere, then go for it. If not, Apple’s store is a decent option if you’re in the market for a slightly cheaper iPhone XS.",
                "id": "k69plbdg"
            },
            "currentTitleWordMap": [
                {
                    "word": "Here's",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "How",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Much",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "You'll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "With",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple-Refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
            "currentAuthorWordMap": [
                {
                    "word": "Brendan",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Hesse",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
            "currentContentWordMap": [
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "now",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sells",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "smartphones",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "through",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "its",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "online",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "store",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "at",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "certified",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "retailers.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "This",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "good",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "news",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "anyone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "who",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "wants",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "upgrade",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "8",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "older,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "might",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "want",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pay",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "full",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "price",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "make",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "full",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "jump",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "11).",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "The",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "barely",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "two",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "years",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "old,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "their",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tech",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "still",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "plenty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "fast,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "so",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "they’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "still",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "solid",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "upgrades.\nThe",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "real",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "question,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "course,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "how",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "much",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buy",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "directly",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Exact",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "final",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pricing",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "depends",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "model",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "configuration",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "go",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "appear",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$200",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cheaper",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "than",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "new,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "while",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "around",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$250",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "The",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "highest",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "expect",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pay",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "each",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "phone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(sans",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tax)",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are:\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$1,099\n\n(Note:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sold",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "by",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked).\nFor",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "reference,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "here’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "quick",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "comparison",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "those",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "same",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "popular",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "third-party",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "aftermarket",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(these",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "based",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "current",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "stock",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "at",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "time",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "writing):\nSwappa\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$659\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$945\n\nBackmarket\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$799-$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$779\n\nGazelle\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$650\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$750\n\nRefurb.me\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$649-$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$749-$1099\n\nAs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "see,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "higher",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "than",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "other",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "third-party",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "seller",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "gamble.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Unless",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’ve",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "bought",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "site",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "seller",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "before,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can’t",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sure",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "device",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "runs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "advertised.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Some",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "offer",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "warranties",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "trade-in",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "programs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "should",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "wind",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "up",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "with",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "bad",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "purchase,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "guarantee.\nConversely,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cleaned",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tested",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "by",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "ensure",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "they",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "meet",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "company’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "quality",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "standards.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "They’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "going",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "flawless",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "brand-new",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "device,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pretty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "close.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "You’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "also",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "get",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "one-year",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "warranty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "extend",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "coverage",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "through",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "AppleCare+.\nTo",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "clear,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "we’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "saying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "that",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "should",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "only",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buy",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "there",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "certain",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "benefits",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "program",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "guarantees",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "its",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "customers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "that",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "make",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "one",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "their",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "less",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "risky,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "even",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "with",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "higher",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "If",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "find",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "better",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "deal",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "trust",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "elsewhere,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "then",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "go",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "If",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "store",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "decent",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "option",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "if",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "in",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "market",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "slightly",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cheaper",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS.",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
            "isInspiration": false,
            "isPencilState": false,
            "isMarkerState": false,
            "isLoading": false,
            "isDisplayFromSaved": false,
            "isPoetryFinished": false,
            "isOpen": false,
            "isNavOpen": false,
            "volNum": 1,
            "step": 0,
            "screenOrientation": "portrait"
        }



        // this.state = {
        //   totalArticles: [],
        //   savedArticles: {},
        //   // Entire Current Article in Original Format (OF)
        //   entireCurrentArticleOF: [],
        //   currentTitleWordMap: [],
        //   currentAuthorWordMap: [],
        //   currentContentWordMap: [],
        //   isInspiration: false,
        //   isPencilState: false,
        //   isMarkerState: false,
        //   isLoading: true,
        //   isDisplayFromSaved: false,
        //   isPoetryFinished: false,
        //   // isOpen: isMobile ? true : false,
        //   isOpen: false,
        //   isNavOpen: false,
        //   volNum: 1,
        //   step: 0,
        //   screenOrientation: 'portrait',
        // };
    }
    isPortraitMode = () => {
        console.log(this.state);
        const { screenOrientation } = this.state;
        return screenOrientation === 'portrait';
    }

    setScreenOrientation = () => {
        if (window.matchMedia("(orientation: portrait)").matches) {
            console.log('orientation: portrait');
            this.setState({
                screenOrientation: 'portrait'
            });
        }

        if (window.matchMedia("(orientation: landscape)").matches) {
            console.log('orientation: landscape');
            this.setState({
                screenOrientation: 'landscape'
            });
        }
    }

    async componentDidMount() {
        window.addEventListener('orientationchange', this.setScreenOrientation);
        window.addEventListener("resize", this.updateDimensions);

        console.log(this.state);

        // const articles = await getArticles();

        // const pendingContents = articles.map(async (article) => getLexperContent(article.url));
        // const fullContents = await Promise.all(pendingContents);

        // articles.forEach((article, i) => {
        //   article.fullContentText = fullContents[i];
        //   article.id = Uniqid();
        // });

        // this.setState({
        //   totalArticles: articles,
        //   entireCurrentArticleOF: {
        //     ...articles[0],
        //     fullContentText: articles[0].fullContentText,
        //     id: articles[0].id,
        //   },
        //   currentTitleWordMap: createWordMap(articles[0].title),
        //   currentAuthorWordMap: createWordMap(articles[0].author),
        //   currentContentWordMap: createWordMap(articles[0].fullContentText),
        //   isLoading: false,
        // });


    }
    componentWillUnmount() {
        window.removeEventListener('orientationchange', this.setScreenOrientation);
        window.removeEventListener("resize", this.updateDimensions);
    }

    updateDimensions = () => {
        this.setState({
            width: window.innerWidth,
            height: window.innerHeight
        });
    }
    pencilState = () => this.setState({
        isPencilState: true,
        isMarkerState: false,
        isDisplayFromSaved: false,
        isInspiration: false,
        isPoetryFinished: false,
    });

    markerState = () => this.setState({
        isPencilState: false,
        isMarkerState: true,
        isDisplayFromSaved: false,
        isInspiration: false,
        isPoetryFinished: false,
    });

    saveState = () => this.setState({
        isPencilState: false,
        isMarkerState: false,
        isDisplayFromSaved: false,
        isInspiration: false,
        isPoetryFinished: true,
    });

    loadExamples = () => {

        this.setState({
            isInspiration: true,
            isDisplayFromSaved: false,
            isPoetryFinished: false,
        });
    };


    loadNewArticle = async () => {
        const { totalArticles, savedArticles, entireCurrentArticleOF } = this.state;

        let randomNum = getRandomNumberUpTo(10);
        if (!(_.isEmpty(savedArticles))) {
            // console.log(savedArticles, "savedArticles")
            while (totalArticles[randomNum].id in savedArticles) {
                // console.log(totalArticles[randomNum].id, " is in ", savedArticles, "so lets find another number")
                randomNum = getRandomNumberUpTo(10);
            }
        }

        this.setState({
            volNum: randomNum,
            isInspiration: false,
            isMarkerState: false,
            isPencilState: false,
            isDisplayFromSaved: false,
            isPoetryFinished: false,
            entireCurrentArticleOF: {
                ...totalArticles[randomNum],
                fullContentText: totalArticles[randomNum].fullContentText,
                id: totalArticles[randomNum].id,
            },
            currentTitleWordMap: createWordMap(totalArticles[randomNum].title),
            currentAuthorWordMap: createWordMap(totalArticles[randomNum].author),
            currentContentWordMap: createWordMap(
                totalArticles[randomNum].fullContentText,
            ),
        });

    };

    saveCurrentArticle = () => {
        const {
            savedArticles,
            currentTitleWordMap,
            currentContentWordMap,
            currentAuthorWordMap,
            entireCurrentArticleOF,
        } = this.state;

        if (!savedArticles[entireCurrentArticleOF.id]) {
            this.setState({
                savedArticles: {
                    ...savedArticles,
                    [entireCurrentArticleOF.id]: {
                        currentTitleWordMap,
                        currentContentWordMap,
                        currentAuthorWordMap,
                        entireCurrentArticleOF,
                    },
                },
                isInspiration: false,
                isMarkerState: false,
                isPencilState: false,
            });
        } else {
            this.setState({
                savedArticles: {
                    ...savedArticles,
                    [entireCurrentArticleOF.id]: {
                        currentTitleWordMap,
                        currentContentWordMap,
                        currentAuthorWordMap,
                        entireCurrentArticleOF,
                        isInspiration: false,
                        isMarkerState: false,
                        isPencilState: false,
                    },
                },
            });
        }
    };

    onClickHandler = (i, category) => {
        const {
            currentTitleWordMap,
            currentAuthorWordMap,
            currentContentWordMap,
        } = this.state;

        switch (category) {
            case 'title':
                this.setState((prevState) => ({
                    currentTitleWordMap: {
                        ...currentTitleWordMap,
                        [i]: {
                            ...currentTitleWordMap[i],
                            isClicked: !prevState.currentTitleWordMap[i].isClicked,
                        },
                    },
                }));
                break;
            case 'author':
                this.setState((prevState) => ({
                    currentAuthorWordMap: {
                        ...currentAuthorWordMap,
                        [i]: {
                            ...currentAuthorWordMap[i],
                            isClicked: !prevState.currentAuthorWordMap[i].isClicked,
                        },
                    },
                }));
                break;
            case 'content':
                this.setState((prevState) => ({
                    currentContentWordMap: {
                        ...currentContentWordMap,
                        [i]: {
                            ...currentContentWordMap[i],
                            isClicked: !prevState.currentContentWordMap[i].isClicked,
                        },
                    },
                }));
                break;
            default:
                break;
        }
    };

    onMouseOverHandler = (i, category) => {
        const {
            currentTitleWordMap,
            currentAuthorWordMap,
            currentContentWordMap,
        } = this.state;

        switch (category) {
            case 'title':
                this.setState({
                    currentTitleWordMap: {
                        ...currentTitleWordMap,
                        [i]: { ...currentTitleWordMap[i], isMouseOver: true },
                    },
                });
                break;
            case 'author':
                this.setState({
                    currentAuthorWordMap: {
                        ...currentAuthorWordMap,
                        [i]: { ...currentAuthorWordMap[i], isMouseOver: true },
                    },
                });
                break;
            case 'content':
                this.setState({
                    currentContentWordMap: {
                        ...currentContentWordMap,
                        [i]: { ...currentContentWordMap[i], isMouseOver: true },
                    },
                });
                break;
            default:
                break;
        }
    };

    downloadCurrentArticle = () => {
        // MAKE A POST REQUEST TO THE BACKEND => SEND THE NECESSARY DATA
        // ON BACKEND, CREATE A PDF BASED ON A TEMPLATE USING THAT DATA
        // MAKE A GET REQUEST TO GET THAT BLOB BACK FROM THE BACKEND
        // CONVERT A BLOB TO A PDF
        // USE FILE-SAVER TO DOWNLOAD THAT PDF
        // const pdf = null;

        // axios.post('/route', { data })
        //   .then(() => {
        //     axios.get('/route')
        //       .then((response) => {
        //         pdf = response.data;
        //       });
        //   });
    }

    onSaveHandler = (i) => {
        const { savedArticles } = this.state;

        const {
            currentAuthorWordMap: clickedAuthorWordMap,
            currentTitleWordMap: clickedTitleWordMap,
            currentContentWordMap: clickedContentWordMap,
        } = savedArticles[i];

        this.setState({
            step: 1,
            isDisplayFromSaved: true,
            isPoetryFinished: false,
            isInspiration: false,
            currentAuthorWordMap: clickedAuthorWordMap,
            currentTitleWordMap: clickedTitleWordMap,
            currentContentWordMap: clickedContentWordMap,
        });
    };

    deleteSavedHandler = (i) => {
        const { savedArticles } = this.state;
        const updatedSavedArticles = _.omit(savedArticles, i);
        this.setState({ savedArticles: { ...updatedSavedArticles } });
    };

    handleOpen = () => {
        this.setState({ isOpen: true });
    };

    handleClose = () => this.setState({ isOpen: false });

    toggleNav = () => this.setState((prevState) => ({ isNavOpen: !prevState.isNavOpen }));

    render() {
        const {
            entireCurrentArticleOF,
            savedArticles,
            isInspiration,
            currentTitleWordMap,
            currentAuthorWordMap,
            currentContentWordMap,
            volNum,
            isPoetryFinished,
            isDisplayFromSaved,
            inspirationImg,
            isPencilState,
            isMarkerState,
            isLoading,
            isNavOpen,
            isOpen,
            step,
        } = this.state;

        const displayComponent = (i) => {
            switch (i) {
                // mobile view instruction
                case 0:
                    this.setState({ step: 0 });
                    break;
                // newspaper view
                case 1:
                    this.setState({ step: 1 });
                    break;
                // saved Newspaper
                case 2:
                    this.setState({ step: 2 });
                    break;
                default:
                    break;
            }
        };

        const componentToBeRendered = () => {
            switch (step) {
                case 0:
                    return (
                        <>
                            <MobileLanding handleOpen={this.handleOpen} />
                            <Modal handleClose={this.handleClose} isOpen={isOpen} />
                        </>
                    );
                case 1:
                    console.log("case 1");
                    return (
                        <Grid item sm={12} md={8} className={styles['newspaper-container']}>
                            <Newspaper
                                volNum={volNum}
                                entireCurrentArticleOF={entireCurrentArticleOF}
                                isPencilState={isPencilState}
                                isMarkerState={isMarkerState}
                                isInspiration={isInspiration}
                                isDisplayFromSaved={isDisplayFromSaved}
                                inspirationImg={inspirationImg}
                                currentTitleWordMap={currentTitleWordMap}
                                currentAuthorWordMap={currentAuthorWordMap}
                                isPoetryFinished={isPoetryFinished}
                                currentContentWordMap={currentContentWordMap}
                                onClickHandler={this.onClickHandler}
                                onMouseOverHandler={this.onMouseOverHandler}
                                loadNewArticle={this.loadNewArticle}
                                loadExamples={this.loadExamples}
                                pencilState={this.pencilState}
                                markerState={this.markerState}
                                saveState={this.saveState}
                                saveCurrentArticle={this.saveCurrentArticle}
                            />
                        </Grid>
                    );
                case 2:
                    return (
                        <Grid
                            item
                            xs={12}
                            md={2}

                            className={styles['saved-newspapers-container']}
                        >
                            <SavedNewspaper
                                onSaveHandler={this.onSaveHandler}
                                savedArticles={savedArticles}
                                volNum={volNum}
                                deleteSavedHandler={this.deleteSavedHandler}
                                isPoetryFinished={isPoetryFinished}
                            />
                        </Grid>
                    );
                default:
                    return null;
            }
        };

        if (_.isEmpty(entireCurrentArticleOF)) {
            return (
                <Grid container className={styles['main-container']}>
                    <Loader
                        loaded={false}
                        scale={2.0}
                        top="50%"
                        left="50%"
                        position="relative"
                        loadedClassName={styles.loader}
                    />
                </Grid>
            );
        }

        if (isMobile) {
            return (
                <>
                    <Button
                        type="button"
                        className={styles['nav-container']}
                        onClick={this.toggleNav}
                    >
                        <DehazeIcon />
                    </Button>
                    <MobileNav
                        toggleNav={this.toggleNav}
                        isNavOpen={isNavOpen}
                        displayComponent={displayComponent}
                    />
                    {
                        componentToBeRendered() || <Modal handleClose={this.handleClose} isOpen={isOpen} />
                    }
                </>
            );
        }

        return (
            <Grid container alignContent='center' justify='center' className={styles['main-container']}>
                <Modal handleClose={this.handleClose} isOpen={isOpen} />
                {console.log(`orientation: from render: isPortraitMode = ${this.isPortraitMode()}`)}
                <Grid item className={styles['bg-showing-vl']} md={2}>
                </Grid>
                <Grid
                    item
                    xs={12}
                    md={8}
                    lg={6}
                    className={classNames(styles['newspaper-container'], isLoading ? styles.loader : '')}
                >
                    <Newspaper
                        volNum={volNum}
                        entireCurrentArticleOF={entireCurrentArticleOF}
                        isPencilState={isPencilState}
                        isMarkerState={isMarkerState}
                        isInspiration={isInspiration}
                        isDisplayFromSaved={isDisplayFromSaved}
                        isPoetryFinished={isPoetryFinished}
                        inspirationImg={inspirationImg}
                        currentTitleWordMap={currentTitleWordMap}
                        currentAuthorWordMap={currentAuthorWordMap}
                        currentContentWordMap={currentContentWordMap}
                        onClickHandler={this.onClickHandler}
                        onMouseOverHandler={this.onMouseOverHandler}
                        loadNewArticle={this.loadNewArticle}
                        loadExamples={this.loadExamples}
                        pencilState={this.pencilState}
                        markerState={this.markerState}
                        saveState={this.saveState}
                        saveCurrentArticle={this.saveCurrentArticle}
                        handleOpen={this.handleOpen}
                    />
                </Grid>
                <Grid item xs={12} sm={2}>
                    <SavedNewspaper
                        onSaveHandler={this.onSaveHandler}
                        savedArticles={savedArticles}
                        volNum={volNum}
                        deleteSavedHandler={this.deleteSavedHandler}
                        isPoetryFinished={isPoetryFinished}
                    />
                </Grid>
            </Grid>
        );
    }
}

export default app;

----------------------
-----------------------
    -----------------------
import React from 'react';
import { Grid, Button } from '@material-ui/core';
import _ from 'lodash';
import Loader from 'react-loader';
import Uniqid from 'uniqid';
import DehazeIcon from '@material-ui/icons/Dehaze';
import classNames from 'classnames';
import { createWordMap, getRandomNumberUpTo } from './utils';
import { Modal, Newspaper, SavedNewspaper, MobileNav, MobileLanding } from './components';
import { getLexperContent, getArticles } from './api';
import styles from './App.module.css';

const isMobile = window.innerWidth <= 959;
// const isMobile = (window.innerWidth <= 850 || window.innerWidth >= 1100);

class App extends React.Component {
    constructor(props) {
        super(props);

        this.state = {

            "totalArticles": [
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "Brendan Hesse",
                    "title": "Here's How Much You'll Save With an Apple-Refurbished iPhone XS",
                    "description": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS…",
                    "url": "https://lifehacker.com/heres-how-much-youll-save-with-an-apple-refurbished-iph-1841138323",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/roo6mjgthvj1jtb5ayr1.jpg",
                    "publishedAt": "2020-01-21T21:00:00Z",
                    "content": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS… [+2300 chars]",
                    "fullContentText": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones  through its online store  and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS Max (or or make the full jump to an iPhone 11). The XS and XS Max are barely two years old, and their tech is still plenty fast, so they’ll still be solid upgrades.\nThe real question, of course, is how much can you save on a refurbished iPhone XS or XS Max you buy directly from Apple. Exact final pricing depends on the model and configuration you go for, but all iPhone XS models appear to be $200 cheaper than buying new, while you’ll save around $250 for an iPhone XS Max. The highest prices you can expect to pay for the 512GB models of each phone (sans tax) are:\n-  512GB iPhone XS:  $999\n-  512GB iPhone XS Max:  $1,099\n\n(Note: all refurbished models sold by Apple are unlocked).\nFor reference, here’s a quick comparison of those same refurbished 512GB unlocked iPhone XS and XS Max models from popular third-party aftermarket sellers (these prices are based on the current stock at time of writing):\nSwappa\n-  iPhone XS:  $659\n-  iPhone XS Max:  $945\n\nBackmarket\n-  iPhone XS:  $799-$999\n-  iPhone XS Max:  $779\n\nGazelle\n-  iPhone XS:  $650\n-  iPhone XS Max:  $750\n\nRefurb.me\n-  iPhone XS:  $649-$999\n-  iPhone XS Max:  $749-$1099\n\nAs you can see, Apple’s prices are higher than other sellers, but buying from a third-party seller can be a gamble. Unless you’ve bought from a site or seller before, you can’t be sure the device runs as advertised. Some sellers offer warranties or trade-in programs should you wind up with a bad purchase, but it’s not a guarantee.\nConversely, all of Apple’s refurbished devices are  cleaned and tested by Apple  to ensure they meet the company’s quality standards. They’re not going to be as flawless as a brand-new device, but it can be pretty close. You’ll also get a one-year warranty on all devices and can extend coverage through AppleCare+.\nTo be clear, we’re not saying that you should only buy a refurbished iPhone XS or XS Max from Apple, but there are certain benefits Apple’s program guarantees its customers that can make buying one of their refurbished devices less risky, even with the higher prices. If you can find a better deal you trust elsewhere, then go for it. If not, Apple’s store is a decent option if you’re in the market for a slightly cheaper iPhone XS.",
                    "id": "k69plbdg"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "Abu Zafar",
                    "title": "How to Take a Screenshot on iPhone X, XS and 11",
                    "description": "If you’ve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhon…",
                    "url": "https://lifehacker.com/how-to-take-a-screenshot-on-iphone-x-xs-and-11-1841064538",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/jz1kqcyhtcozukg1wike.jpg",
                    "publishedAt": "2020-01-21T14:30:00Z",
                    "content": "If youve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhone… [+330 chars]",
                    "fullContentText": "If you’ve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhone X redesign in 2017, the button combination has changed. In the short video above, I show you two ways to take a screenshot on a modern iPhone.\nQuick Fix is a video series where I tackle your most commonly asked tech questions in 90 seconds or less. If you have a question you want me to answer, leave a comment below or email me.",
                    "id": "k69plbdh"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "You Can't Stream 'Star Trek: Picard' on Linux Without This Workaround",
                    "description": "It’s 2020, and you think that it would be easy to stream any content you want on any device you want—especially that brand-new Picard series on CBS All Access. If you’re an ubergeek who loves both Star Trek and Linux, you’re out of luck. Thanks to the joys of…",
                    "url": "https://lifehacker.com/you-cant-stream-star-trek-picard-on-linux-without-this-1841327097",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/v4nesxfyduqgoy7qpdct.png",
                    "publishedAt": "2020-01-29T19:00:00Z",
                    "content": "Its 2020, and you think that it would be easy to stream any content you want on any device you wantespecially that brand-new Picard series on CBS All Access. If youre an ubergeek who loves both Star Trek and Linux, youre out of luck. Thanks to the joys of dig… [+2005 chars]",
                    "fullContentText": "It’s 2020, and you think that it would be easy to stream any content you want on any device you want—especially that brand-new  Picard  series on CBS All Access. If you’re an ubergeek who loves both  Star Trek  and Linux, you’re out of luck. Thanks to the joys of digital rights management, or DRM, you’re unable to use CBS’ All Access service to stream Picard on Linux.\nIn a cruel twist, not only can you not stream the show, but you  can  stream all the bullshit that comes wrapped around it. As Ars Technica’s Jim Salter  describes  :\n“Moving back to one of my own PCs, I was quickly able to confirm the issue: trailers autoplay properly, and even the ads work—but the actual content won’t play on a Linux desktop PC on any browser including Google Chrome. Diving into the Chrome Web Console, we can see HTTP 400 (Bad Request) errors when the browser attempts to fetch a license from CBS’  Widevine  back end.”\nThe more tech-savvy among you are probably assuming that you can just change your browser’s user agent to fool CBS All Access into thinking you’re using a supported platform. Not so. (Salter tried that, too.) For whatever reason, Widevine and CBS All Access don’t play nice with Linux, even though other streaming services that use Widevine have no issues.\nThe only workaround to this vexing problem, aside from watching CBS All Access on a different device entirely, is to cancel your regular CBS All Access subscription and instead subscribe to the service via  Amazon Prime Video  . Yes, you’ll need to be an existing Amazon Prime member for this to work (in the U.S., at least), which will already set you back $120 annually. After that, adding CBS All Access costs the same as if you were subscribing standalone: $6/mo for limited commercials or $10/mo for commercial-free streaming.\nThat is, however, slightly more than you’d pay if you signed up for an annual plan  via CBS itself  ($60/year for limited commercials and and $100/year for no commercials). Amazon doesn’t offer these options, so this route is best if you’re only planning to binge a show or two and then cancel your service. If you’re looking to stick with CBS All Access for the long haul, you might just want to try not watching it on Linux; a tablet, phone, or app on your Smart TV will save you more cash.",
                    "id": "k69plbdi"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "Josh Ocampo",
                    "title": "Why You Shouldn't Let Celebrities Text You Via Community",
                    "description": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on socia…",
                    "url": "https://lifehacker.com/why-you-shouldnt-let-celebrities-text-you-via-community-1841259489",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/sgh5mxng1qcls0vqmmp6.jpg",
                    "publishedAt": "2020-01-27T18:30:00Z",
                    "content": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on socia… [+1776 chars]",
                    "fullContentText": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on social media, they’re likely about to target you even more closely.\nRecently, if you’ve ever stumbled across a tweet or Instagram in which a celebrity posts their “phone number” so that they can interact with and text you directly, it’s not what you think—instead of entering into a personal chat with a celebrity, you’ve  actually  subscribed to a newsletter of automated updates that’ll be sent by text.\nBack in October, reporter Taylor Lorenz described how it works in detail for the  New York Times  . If you “text” a celebrity using this number, you’re likely signing up for a service run by a tech-startup called  Community  ; they help manage direct messaging with large audiences on behalf of celebrities and other well-known figures, as a way of “monetizing” their social network.\nOnce you opt-in by way of text, what kind of updates might you receive? Well, it depends. Sometimes, it’s just a random string of photos and texts, as Lorenz received from actor Ashton Kutcher. I signed up for  Pod Save America’s  Jon Favreau’s updates and just received a bunch of updates about their tour.\nIf you’re really, really lucky, you  could  a text back from the celebrity in question—but don’t count on it. Given the number of messages they likely receive, it would seem like a huge effort just to respond to every text. The vast majority of the time, you’re likely only at the receiving end of these automated updates.\nIf you’re a particular fan of a celebrity, maybe signing up isn’t such a bad idea. But we’ll do you a favor and tell you now that it probably isn’t worth it. The odds are incredibly low that you’ll receive a text that isn’t just an automated update, and while  Community ensures  that they will not sell your personal details to other third parties, well,  we’ve all heard that before  , haven’t we?",
                    "id": "k69plbdj"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "Help! I Bumped My PC and Now It Won't Turn On",
                    "description": "Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could…",
                    "url": "https://lifehacker.com/help-i-bumped-my-pc-and-now-it-wont-turn-on-1841360774",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/o2oyoxjrp7m2htuheipk.jpg",
                    "publishedAt": "2020-01-31T14:00:00Z",
                    "content": "Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could… [+5747 chars]",
                    "fullContentText": "Tech 911  Do you have a tech question keeping you up at night? We'd love to answer it! Email david.murphy@lifehacker.com with \"Tech 911\" in the subject line.  Prev  Next Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could end it all.\nYour desktop PC is a bit more protected, in theory, since it probably lives in a bulky case. That doesn’t mean it’s invulnerable. Though we weren’t sent this question by a Lifehacker reader, as is custom for the  Tech 911  column, I stumbled across this call for help on Reddit and couldn’t resist highlighting it in this this week’s edition. It’s just that good.\nRedditor  u/dyingquickly  describes  :\n“I lightly bumped my computer it shot off and makes no attempt to boot when I hit the power button. I know my PSU is working because I have a tester. I’ve triple checked all my connections. Does anyone know what the problem might be? The light of the on button doesn’t even appear, it’s as if the button isn’t working but i don’t know what the problem is.”\nGenerally speaking, when your computer zaps off like that, it’s a pretty good indication that you’re going to have to do some troubleshooting. If you’re lucky, you’re only dealing with a random power outage or loose cable—possibly even a button on your surge protector that you brushed up against. You should be able to correct the problem and turn your system back on again, suffer a quick disk scan, and resume whatever it is you were doing. (Here’s hoping your work or  browser tabs  autosaved.)\nWhen your system  doesn’t  turn on is when this process really gets fun.\n\nCHECK FOR LOOSE CABLES\nSuppose you bang into your desktop PC with your chair, your foot, or your stupid but loveable pet runs into it. We’ll classify this as a “hard” bump, not a gentle dusting. If your desktop PC won’t turn back on—as in, it won’t even POST—my first assumption is a crucial cable has fallen out of a connector  just enough  to cause an issue.\nThis is going to be annoying, but I recommend popping open your desktop PC and making sure everything looks right. If some cable isn’t connected in some awkward way, that might be the culprit—though your motherboard’s primary power cables  should  have locking mechanisms that make this impossible.  I wouldn’t rule anything out  . In fact, I’d start with them: the fat 24-pin cable and the smaller 4/8-pin cable.\n\nSTART DISCONNECTING AND RECONNECTING CRITICAL CABLES\nIf a loose cable isn’t immediately apparent, you might want to take the more annoying step of unplugging everything that’s plugged in and reconnecting it. I’d start with more critical cables first—those going into your motherboard, graphics card, and modular power supply (if applicable). You can try your SATA cables, too, as well as the power connections for your hard drive(s). (Though your motherboard should still be able to POST without a hard drive connected, let’s not take any chances.)\nYou’ll also want to check your system’s tiny front-panel connectors. While a loose front-panel cable would explain why you can’t turn your system back on, you’ll also likely be able to surmise this is the issue if your motherboard is otherwise powered on (via any lights you see). If your motherboard has its own power button you can use to start your system, and pressing that makes everything work correctly, then you’re likely dealing with some issue with your front-panel cables.\nAlso, make sure you’re checking the rear of your system. There are plenty of cables back there—the power cable, especially—so it’s possible that something got jiggered out of place in such a way as to prevent your computer from turning on. Again, this might feel like try-anything troubleshooting, but I’m willing to bet that the answer to this issue is going to be some random cable you wouldn’t have normally thought about in your troubleshooting. Heck, you can even try resetting the CMOS battery; I doubt it’s the culprit, but it’s worth exploring if your other ideas don’t pan out.\nIt’s also possible that you bumped something out of place in your case that is now causing some kind of short-circuit with your motherboard. That could be a cable; that could be a screw that you accidentally left in your system when you were building it. However, short of eyeballing the problem, taking your system apart and rebuilding it is likely going to be the best way to address this. Given how long that might take to do, I’d save it as a final option.\nAnd if you go this route, be  methodical  . Strip your system to the bare components. Take the CPU and cooler off your motherboard, remove the RAM, and unplug everything (to name a few steps). Once you’re staring at a bunch of components, plug everything back in as if you’re  building a computer for the first time  . Connect the basics—your motherboard, your RAM, and your video card (if applicable)—and try powering on your system. Can you make it into the BIOS? If so, power down your PC and start adding your other hardware one at a time, until you can deduce what, if anything, is causing issues.\n\nTRY FIGURING OUT WHICH COMPONENT HAS FAILED\nAnd if you still can’t power on your PC, you might want to phone a friend. Slap your hard drive into their system or external enclosure to confirm your data is safe (and back it up, if need be)? See if you can borrow an extra power supply to use with your core components. Do they have any extra RAM you could try? Will your video card work on their system?\nAt this point, you’re likely trying to pinpoint faulty hardware, since odds are getting higher that you actually did some kind of damage to something inside your system. Hopefully you don’t have to reach this point; I’ve been there, and it’s not a fun bit of detective work.\nDo you have a tech question keeping you up at night? Tired of troubleshooting your Windows or Mac? Looking for advice on apps, browser extensions, or utilities to accomplish a particular task? Let us know! Tell us in the comments below or email  .",
                    "id": "k69plbdk"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "Change These Facebook Settings to Protect Your Photos From Facial Recognition Software",
                    "description": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesn’t make this weekend’s report from The New York Times any less dam…",
                    "url": "https://lifehacker.com/change-these-facebook-settings-to-protect-your-photos-f-1841131425",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/kgodep1gxupxb0lqj9je.png",
                    "publishedAt": "2020-01-21T20:00:00Z",
                    "content": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesnt make this weekends report from The New York Times any less damni… [+4316 chars]",
                    "fullContentText": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesn’t make this weekend’s report from  The New York Times  any less damning, it’s a great reminder about how important it is to  really  go through the settings for your various social networks and limit how your content is, or can be, accessed by anyone.\nI won’t get too deep into the  Times’  report; it’s worth reading on its own, since it involves a company (Clearview AI) scraping more than three billion images from millions of websites, including Facebook, and creating a facial-recognition app that does a pretty solid job of identifying people using images from this massive database.\nEven though Clearview’s scraping techniques technically violate the terms of service on a number of websites, that hasn’t stopped the company from acquiring images  en masse.  And it keeps whatever it finds, which means that turning all your online data private isn’t going to help if Clearview has already scanned and grabbed your photos.\nStill, something is better than nothing. On  Facebook  , likely the largest stash of your images, you’re going to want to visit  Settings >  Privacy  and look for the option described:  “Do you want search engines outside of Facebook to link to your profile?”\nTurn that off, and Clearview won’t be able to grab your images. That’s not the setting I would have expected to use, I confess, which makes me want to go through all of my social networks and rethink how the information I share with them flows out to the greater web.\n\nLOCK DOWN YOUR FACEBOOK EVEN MORE WITH THESE SETTINGS\nSince we’re already here, it’s worth spending a few minutes wading through Facebook’s settings and making sure as much of your content is set to friends-only as possible. That includes changing  “Who can see your future posts”  to “friends,” using the  “Limit Past Posts”  option to change everything you’ve previously posted to friends-only, and making sure that only you can see your friends list—to prevent any potential scraping and linking that some third-party might attempt. Similarly, make sure only your friends (or friends of friends) can look you up via your email address or phone number. (You never know!)\nYou should then visit the  “  Timeline and Tagging  ”  settings page and make a few more changes. That includes only allowing friends to see what other people post on your timeline, as well as posts you’re tagged in. And because I’m a bit sensitive about all the crap people tag me in on Facebook, I’d turn on the “Review” options, too. That won’t help your account from being scraped, but it’s a great way to exert more control over your timeline.\nFinally, even though it also doesn’t prevent companies from scraping your account, pull up the  “  Public posts  ”  section of Facebook’s settings page and limit who is allowed to follow you (if you desire). You should also restrict who can comment or like your public information, like posts or other details about your life you share openly on the service.\n\nONCE I FIX FACEBOOK, THEN WHAT?\nHere’s the annoying part. Were I you, I’d take an afternoon or evening and write out all the different places I typically share snippets of my life online. For most, maybe that’s probably a handful of social services: Facebook, Instagram, Twitter, YouTube, Flickr, et cetera.\nOnce you’ve created your list, I’d dig deep into the settings of each service and see what options you have, if any, for limiting the availability of your content. This might run contrary to how you use the service—if you’re trying to gain lots of Instagram followers, for example, locking your profile to “private” and requiring potential followers to request access might slow your attempts to become the next big Insta-star. However, it should also prevent anyone with a crafty scraping utility to mass-download your photos (and associate them with you, either through some fancy facial-recognition tech, or by linking them to your account).\nWill you be able to fully prevent someone from finding a way to build the next big database of searchable images? Probably not. But the most important lesson we can take from situations like these—Clearview AI’s...well...existence—is that you should understand the nuances of the services you share your information to, and you should always use your settings to restrict access to your data in whatever way you feel most comfortable. Were I you, I’d err on the side of “let trusted people see what I do instead of anyone on the internet,” but that’s just me.",
                    "id": "k69plbdl"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Lifehacker.com"
                    },
                    "author": "David Murphy",
                    "title": "I Lost Nine Years of Photos by Locking Myself Out of My Google Account",
                    "description": "I normally use this weekly column to answer people’s technology-themed questions. This week, I’m taking a slight departure, because I think sharing a reader’s story is important—even though there’s not really much I, nor Google, can do in the case of her lock…",
                    "url": "https://lifehacker.com/i-lost-nine-years-of-photos-by-locking-myself-out-of-my-1841178170",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/qsqfi9lclqdlo3adj7ih.jpg",
                    "publishedAt": "2020-01-24T14:00:00Z",
                    "content": "I normally use this weekly column to answer peoples technology-themed questions. This week, Im taking a slight departure, because I think sharing a readers story is importanteven though theres not really much I, nor Google, can do in the case of her locked Go… [+6981 chars]",
                    "fullContentText": "Tech 911  Do you have a tech question keeping you up at night? We'd love to answer it! Email david.murphy@lifehacker.com with \"Tech 911\" in the subject line.  Next I normally use  this weekly column  to answer people’s technology-themed questions. This week, I’m taking a slight departure, because I think sharing a reader’s story is important—even though there’s not really much I, nor Google, can do in the case of her locked Google account. There’s a lot we can learn from her example, and a few items you should check to make sure this frustrating issue never happens to you.\nLifehacker reader  Cathryn  writes:\nI stupidly forgot my Google account password back in November 2019, I had only just changed it. I asked them to email to text me a verification code, like I’d done so many times before, however I stupidly added the 2 step verification on my account.\nThe problem was I inputted an incorrect recovery email address, I missed 2 digits off my email. Therefore the 2 step verification wouldn’t work. Google locked my account, I also broke my mobile phone screen days later that I ended up buying a new handset but the same mobile phone number.\n[...]\nI only get asked 1 security question “what was your first phone number” I picked that question myself in the security settings a long time ago. Then I get asked to give them an email they can contact me on, then a few days later I receive an email stating they don’t have enough evidence so unfortunately they can’t open my account.\nI never get asked any questions such as when was your account created. I’ve tried recovery options from my laptop, kindle ect that used the same gmail account plus it’s all from my home IP address.\nI get it’s my fault for initially forgetting my password, but google security settings are flawed as when I put a recovery email they should send an automatic email confirmation to make sure it’s correct due to human error. I’ve lost lots of important document and 9 years of Google photos with some pictures of loved ones that I’ll never get back.\nI present this reader story as a cautionary tale. First, whenever you’re dealing with two-factor authentication or any kind of account-recovery options—whether you’re inputting your email or phone number to set it up, or you’re copying down precious backup codes—it’s critical that you triple-check this information. That’s not because there’s no way to change it; you can obviously change details like your email address or phone number if you have access to your account. However, you’re less likely to this because most people assume that everything is on the up and up once they’ve set up their accounts. And if it isn’t, and you left a typo in a critical account recovery option, you’re going to have a mess of a time when you try to restore access.\nGoogle makes this entire situation particularly frustrating because they, a giant search company that practically prints money, have no desire to set up a customer service center that Google users can call whenever they have issues with Google’s services. Regular (free) Google users get a  Help Center  and some community forums, but that’s it.\nAnd while I’m a little snarky about this, I do see Google’s point of view. First off, most people aren’t paying Google a dime for the services they’re using. Yes, Google is doing everything it can to extract data from you so it can better target you with relevant advertising, but most people aren’t coughing up cash for the privilege of using Gmail, for example. You might pay Google a little per month  for some extra storage  , but that’s about it. Why dedicate a not-so-insignificant amount of resources to a service center for free services?\nSecond, and more importantly, Google’s lack of one-to-one help for issues like account recovery is actually a security measure in itself. Consider the alternative: If someone sort-of knew your email address, phone number, and/or mailing address, in combination with some information they’ve pilfered from one of the many data breaches out there, would you want them to be able to convince a Google customer service representative that they are actually you?\nGiving everyone the same, generic account recovery process, and letting nobody have access to any special favors or extra support, makes it harder to get into a locked account when you have unique issues, sure. It also makes it harder for others to get into  your  account. I’m not saying I’m a fan of this trade-off—because real people  do  have complex issues that need special help—but it does help limit an unscrupulous person’s ability to social engineering their way into your Google account.\nThis puts the burden on you to make sure that all of your recovery mechanisms for your Google account are accurate. Even if you’ve already set them up and are feeling pretty good about it, I recommend taking a quick few minutes to double-check. It never hurts, because you don’t want to deal with the alternative of not being able to access your account in the event of some major mess or security breach.\nTo get started, visit the  Security  section of your Google account settings. Scroll down a bit to the “Ways we can verify it’s you” list of questions and make sure your phone number, recovery email address, and security question are all accurate.\nNext, scroll up a smidge and click on  “  2-Step Verification  .”  You’ll have to enter your password again. On the subsequent screen, make sure that you’ve turned  on  Google’s two-step verification to better protect your account. Assuming you have, scroll down a bit and click on “Show Codes” under the “Backup codes” section.\nCopy. These. Codes. Down.  To paraphrase Gandalf, keep them secret and safe; you’ll use them to log back into your account if, for whatever reason, you can’t access the device(s) you’ve set up for two-step verification.\nFinally, go visit the  Personal info  section of your Google account settings and double-check that your actual contact information is correct: email(s) and phone number(s). While you’re here, you might as well also make sure you’ve entered your correct birthday, too.\nAnd, of course, the best way to make sure you never forget your passwords to anything is to use  a strong password manager  . Even if it costs you a few bucks a month, it’s worth every penny. Not only does this make it easy to keep track of your passwords, but you can use stronger, more secure passwords that are  very  difficult to guess or brute-force. Also, a great password manager will let you know whenever your saved accounts are compromised in a data breach (which should be less of an issue if you’re using unique passwords for each service, but people can get a little lazy about this).\nThat’s my advice to everyone, and it applies to more than just Google. If a service is important to you, make sure you know the process of how you’ll get back into your account if you’re ever locked out. Then, check your data to make sure any errant typos won’t come back to haunt you, and make sure you’ve saved any backup codes or any other information you need in order to regain access to a lost or locked account. You’ll be glad you did now, if (or when) you have to deal with this later.\nDo you have a tech question keeping you up at night? Tired of troubleshooting your Windows or Mac? Looking for advice on apps, browser extensions, or utilities to accomplish a particular task? Let us know! Tell us in the comments below or email  .",
                    "id": "k69plbdm"
                },
                {
                    "source": {
                        "id": null,
                        "name": "Theinventory.com"
                    },
                    "author": "Tercius, Ana Suarez, and Corey Foster on Kinja Deals, shared by Ana Suarez to Lifehacker",
                    "title": "Monday's Best Deals: Sony ANC Headphones, Tile Gold Box, Witcher 3: Wild Hunt, and More",
                    "description": "A windshield wiper sale, 2TB Seagate Firecuda, foam roller, and a supplement Gold Box lead of Monday’s best deals from around the web. Read more...",
                    "url": "https://kinjadeals.theinventory.com/mondays-best-deals-sony-anc-headphones-tile-gold-box-1841258693",
                    "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/fkz6rbhgodm1ezkghyji.jpg",
                    "publishedAt": "2020-01-27T15:30:00Z",
                    "content": "A windshield wiper sale, 2TB Seagate Firecuda, foam roller, and a supplement Gold Box lead of Mondays best deals from around the web.\r\nBookmark Kinja Deals and follow us on Twitter to never miss a deal.\r\neBay is offering one of the best deals on the best nois… [+14280 chars]",
                    "fullContentText": {},
                    "id": "k69plbdn"
                },
                {
                    "source": {
                        "id": "engadget",
                        "name": "Engadget"
                    },
                    "author": "Richard Lawler",
                    "title": "The Morning After: Meet Cadillac's tech-heavy new Escalade",
                    "description": "Hey, good morning! You look fabulous. Good morning! Since there's nothing at all wonky to report with Election 2020 results and incredibly flawed apps that were shoddily included in the primary process, let me point you to your next 4K demo reel. It's from 18…",
                    "url": "https://www.engadget.com/2020/02/05/2021-escalade-morning-after/",
                    "urlToImage": "https://o.aolcdn.com/images/dims?thumbnail=1200%2C630&quality=80&image_uri=https%3A%2F%2Fo.aolcdn.com%2Fimages%2Fdims%3Fresize%3D2000%252C2000%252Cshrink%26image_uri%3Dhttps%253A%252F%252Fs.yimg.com%252Fos%252Fcreatr-uploaded-images%252F2020-02%252F73cf1810-4810-11ea-bfbb-f06a233254c6%26client%3Da1acac3e1b3290917d92%26signature%3D3fa8c987b941255de22db64185ddf92abbe04f9b&client=amp-blogside-v2&signature=303b1045fccfa43bdb58bee9ac63b38f8b8e5057",
                    "publishedAt": "2020-02-05T12:30:00Z",
                    "content": "Games are capped at 1080p and 60 fps.NVIDIA's GeForce Now cloud gaming service is live\r\nAfter seven years of tweaking its delivery systems and gathering beta feedback, NVIDIA has finally unveiled the consumer version of its game-streaming ecosystem. GeForce N… [+3181 chars]",
                    "fullContentText": "Games are capped at 1080p and 60 fps.  NVIDIA's GeForce Now cloud gaming service is live\nAfter seven years of tweaking its delivery systems and gathering beta feedback, NVIDIA has  finally unveiled the consumer version of its game-streaming ecosystem  . GeForce Now streams games of all sizes to PC, Mac, Android and the NVIDIA Shield, and it works with players' existing libraries on Steam, the Epic Games Store and all other digital platforms.\nA free option allows folks to connect to NVIDIA's servers and play for one hour at a time. After an hour, these players will need to reconnect and potentially wait for a spot to open up on the servers. In the paid tier, players go to the front of the line when logging on, they'll stay online for up to six hours and get access to RTX hardware.\nAnd release dates for some highly anticipated shows.  Disney+ has 28.6 million subscribers so far\nIn Disney's first earnings report since the launch of Disney+, the company announced  how many people have signed up for its streaming service  . As of Monday, the number is 28.6 million, which CEO Bob Iger said he doesn't expect to change significantly until this fall. That's when October's  The Mandalorian  season two arrives, as well as MCU-spinoff shows  Falcon and the Winter Soldier  (August) and  Wandavision  (December).\nIt's OK, though, they apologized.  Google Photos bug sent private videos to strangers\nIf you used Google Takeout to back up your Photos files from November 21st to 25th, 2019, a stranger out there may have a copy of your videos. During that period, the email reads, \"some videos in Google Photos were incorrectly exported to unrelated users' archives.\" That means  someone out there may have a copy of your (hopefully wholesome) videos  , and you may have a copy of someone else's, as well.\nDon't forget the augmented reality navigation system.  2021 Cadillac Escalade packs 38 inches of curved OLED screens and Super Cruise\nCadillac is introducing a new Escalade later this year, and inside you'll find three curved OLED screens in the dashboard. There's a 7.2-inch touch panel to the driver's left, a 14.2-inch screen info cluster behind the steering wheel and the 16.9-inch infotainment system. With \"perfect\" black levels and bright colors, the screens don't need hoods over them to block out the light.\nThe luxury SUV line also packs five USB ports (three of them are USB-C), wireless charging, an optional 36-speaker audio system that uses mics to let front and rear passengers communicate, Super Cruise driving assist that can now change lanes by itself, NFC and electric motors that help the doors close quietly.  Check out all the details and get a closer look here  .\nThe show is scheduled to start February 24th.  LG pulls out of Mobile World Congress over coronavirus concerns\nAs concerns over a growing coronavirus outbreak continue to mount, LG has decided that the Barcelona-based trade show  simply isn't worth the risk  . Instead, LG will hold separate events in the \"near future\" to reveal its batch of 2020 smartphones, which is expected to include a sequel to last year's ambitious, underwhelming G8 ThinQ.\n\nBUT WAIT, THERE'S MORE...\n-  Iowa Democrats say sloppy app code delayed caucus results\n-  Sony sets prices for (most of) its 2020 4K LED TVs\n-  'Altered Carbon' season two teaser shows Anthony Mackie as the 'new' hero\n-  Now T-Mobile and Sprint can verify calls across their networks to prevent robocalls\n-  KFC slips ads into Spotify Premium through artist profiles\n-  Dyson patents a wearable air purifier that doubles as headphones\n-  SpaceX asks permission to take Starship on a high-altitude test flight\n-  Rockstar Games co-founder Dan Houser is leaving the company\n\nThe Morning After is a new daily newsletter from Engadget designed to help you fight off FOMO. Who knows what you'll miss if you don't  Subscribe  .\nCraving even more?  Like us on Facebook  or  Follow us on Twitter  .\nHave a suggestion on how we can improve The Morning After?  Send us a note  .\nIn this article:  av  ,  business  ,  design  ,  entertainment  ,  gadgetry  ,  gadgets  ,  gear  ,  internet  ,  mobile  ,  space  ,  themorningafter  ,  transportation  All products recommended by Engadget are selected by our editorial team, independent of our parent company. Some of our stories include affiliate links. If you buy something through one of these links, we may earn an affiliate commission.  Comments",
                    "id": "k69plbdo"
                },
                {
                    "source": {
                        "id": "engadget",
                        "name": "Engadget"
                    },
                    "author": "Rachel England",
                    "title": "Yubico is making it easier for businesses to buy its YubiKeys",
                    "description": "A growing number of companies are looking at hardware authentication security keys as a trusted and convenient way to protect sensitive corporate data. Indeed, Google has recently launched an open source project to help advance the uptake of this technology. …",
                    "url": "https://www.engadget.com/2020/02/04/yubico-business-yubikey-enterprise-security-key/",
                    "urlToImage": "https://o.aolcdn.com/images/dims?thumbnail=1200%2C630&quality=80&image_uri=https%3A%2F%2Fo.aolcdn.com%2Fimages%2Fdims%3Fcrop%3D1000%252C656%252C0%252C147%26quality%3D85%26format%3Djpg%26resize%3D1600%252C1050%26image_uri%3Dhttps%253A%252F%252Fs.yimg.com%252Fos%252Fcreatr-uploaded-images%252F2019-06%252Fdffe5260-8e13-11e9-b63f-ce5f582bb97a%26client%3Da1acac3e1b3290917d92%26signature%3De8a7567296fdb4cf66936f0c2064ac2f77b7c80d&client=amp-blogside-v2&signature=b0997fd1d0f76bb41ba86eca944320c591707ef4",
                    "publishedAt": "2020-02-04T12:00:00Z",
                    "content": "The company is initially offering two services: YubiEnterprise Subscription (which is available now), and YubiEnterprise Delivery (which is slated for release in spring). YubiEnterprise Subscription is an annual per-user pricing model that allows companies to… [+1265 chars]",
                    "fullContentText": "The company is initially offering two services: YubiEnterprise Subscription (which is available now), and YubiEnterprise Delivery (which is slated for release in spring). YubiEnterprise Subscription is an annual per-user pricing model that allows companies to subscribe to a specific service tier based on how many users they need to support. The idea is that by moving away from a per-key purchasing model, organizations can better predict future spending and upgrades. This one is designed for businesses with 750 YubiKey users or more.\nThe second offering, YubiEnterprise Delivery, takes care of the actual logistics of key distribution. Via a dedicated Yubico portal (or software that can be integrated directly into existing IT software using public APIs) IT administrators can keep a track of who needs access to which keys, and the service will manage inventory, ship keys to the relevant people, monitor delivery and manage returns or replacements.\nYubico is one of the biggest players in the security key business -- and its tech is widespread across  Windows  and  iOS  devices -- so it's not a surprise that it's the first one to make this kind of offering. And it's much-needed -- businesses overall are gradually waking up to the benefits of security keys, but until now the logistical challenges in implementing them means they've largely been the sole preserve of smaller companies. Yubico's enterprise service opens the tech up to much larger companies, which will no doubt help accelerate its adoption overall.\nIn this article:  2FA  ,  business  ,  design  ,  enterprise  ,  gear  ,  hardware authentication  ,  personal computing  ,  personalcomputing  ,  security  ,  security key  ,  service  ,  Yubico  ,  Yubikey\n\nAll products recommended by Engadget are selected by our editorial team, independent of our parent company. Some of our stories include affiliate links. If you buy something through one of these links, we may earn an affiliate commission.\n\nComments\n\n47  Shares\n\nShare\n\nShare",
                    "id": "k69plbdp"
                }
            ],
            "savedArticles": {},
            "entireCurrentArticleOF": {
                "source": {
                    "id": null,
                    "name": "Lifehacker.com"
                },
                "author": "Brendan Hesse",
                "title": "Here's How Much You'll Save With an Apple-Refurbished iPhone XS",
                "description": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS…",
                "url": "https://lifehacker.com/heres-how-much-youll-save-with-an-apple-refurbished-iph-1841138323",
                "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/roo6mjgthvj1jtb5ayr1.jpg",
                "publishedAt": "2020-01-21T21:00:00Z",
                "content": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS… [+2300 chars]",
                "fullContentText": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones  through its online store  and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS Max (or or make the full jump to an iPhone 11). The XS and XS Max are barely two years old, and their tech is still plenty fast, so they’ll still be solid upgrades.\nThe real question, of course, is how much can you save on a refurbished iPhone XS or XS Max you buy directly from Apple. Exact final pricing depends on the model and configuration you go for, but all iPhone XS models appear to be $200 cheaper than buying new, while you’ll save around $250 for an iPhone XS Max. The highest prices you can expect to pay for the 512GB models of each phone (sans tax) are:\n-  512GB iPhone XS:  $999\n-  512GB iPhone XS Max:  $1,099\n\n(Note: all refurbished models sold by Apple are unlocked).\nFor reference, here’s a quick comparison of those same refurbished 512GB unlocked iPhone XS and XS Max models from popular third-party aftermarket sellers (these prices are based on the current stock at time of writing):\nSwappa\n-  iPhone XS:  $659\n-  iPhone XS Max:  $945\n\nBackmarket\n-  iPhone XS:  $799-$999\n-  iPhone XS Max:  $779\n\nGazelle\n-  iPhone XS:  $650\n-  iPhone XS Max:  $750\n\nRefurb.me\n-  iPhone XS:  $649-$999\n-  iPhone XS Max:  $749-$1099\n\nAs you can see, Apple’s prices are higher than other sellers, but buying from a third-party seller can be a gamble. Unless you’ve bought from a site or seller before, you can’t be sure the device runs as advertised. Some sellers offer warranties or trade-in programs should you wind up with a bad purchase, but it’s not a guarantee.\nConversely, all of Apple’s refurbished devices are  cleaned and tested by Apple  to ensure they meet the company’s quality standards. They’re not going to be as flawless as a brand-new device, but it can be pretty close. You’ll also get a one-year warranty on all devices and can extend coverage through AppleCare+.\nTo be clear, we’re not saying that you should only buy a refurbished iPhone XS or XS Max from Apple, but there are certain benefits Apple’s program guarantees its customers that can make buying one of their refurbished devices less risky, even with the higher prices. If you can find a better deal you trust elsewhere, then go for it. If not, Apple’s store is a decent option if you’re in the market for a slightly cheaper iPhone XS.",
                "id": "k69plbdg"
            },
            "currentTitleWordMap": [
                {
                    "word": "Here's",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "How",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Much",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "You'll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "With",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple-Refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
            "currentAuthorWordMap": [
                {
                    "word": "Brendan",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Hesse",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
            "currentContentWordMap": [
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "now",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sells",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "smartphones",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "through",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "its",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "online",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "store",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "at",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "certified",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "retailers.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "This",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "good",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "news",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "anyone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "who",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "wants",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "upgrade",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "8",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "older,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "might",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "want",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pay",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "full",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "price",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "make",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "full",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "jump",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "11).",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "The",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "barely",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "two",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "years",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "old,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "their",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tech",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "still",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "plenty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "fast,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "so",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "they’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "still",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "solid",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "upgrades.\nThe",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "real",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "question,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "course,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "how",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "much",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buy",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "directly",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Exact",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "final",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pricing",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "depends",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "model",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "configuration",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "go",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "appear",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$200",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cheaper",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "than",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "new,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "while",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "around",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$250",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "The",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "highest",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "expect",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pay",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "each",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "phone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(sans",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tax)",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are:\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$1,099\n\n(Note:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sold",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "by",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked).\nFor",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "reference,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "here’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "quick",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "comparison",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "those",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "same",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "popular",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "third-party",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "aftermarket",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(these",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "based",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "current",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "stock",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "at",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "time",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "writing):\nSwappa\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$659\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$945\n\nBackmarket\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$799-$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$779\n\nGazelle\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$650\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$750\n\nRefurb.me\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$649-$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$749-$1099\n\nAs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "see,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "higher",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "than",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "other",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "third-party",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "seller",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "gamble.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Unless",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’ve",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "bought",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "site",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "seller",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "before,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can’t",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sure",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "device",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "runs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "advertised.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Some",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "offer",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "warranties",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "trade-in",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "programs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "should",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "wind",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "up",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "with",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "bad",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "purchase,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "guarantee.\nConversely,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cleaned",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tested",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "by",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "ensure",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "they",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "meet",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "company’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "quality",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "standards.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "They’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "going",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "flawless",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "brand-new",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "device,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pretty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "close.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "You’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "also",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "get",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "one-year",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "warranty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "extend",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "coverage",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "through",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "AppleCare+.\nTo",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "clear,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "we’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "saying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "that",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "should",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "only",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buy",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "there",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "certain",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "benefits",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "program",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "guarantees",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "its",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "customers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "that",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "make",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "one",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "their",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "less",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "risky,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "even",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "with",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "higher",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "If",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "find",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "better",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "deal",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "trust",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "elsewhere,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "then",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "go",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "If",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "store",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "decent",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "option",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "if",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "in",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "market",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "slightly",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cheaper",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS.",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
            "isInspiration": false,
            "isPencilState": false,
            "isMarkerState": false,
            "isLoading": false,
            "isDisplayFromSaved": false,
            "isPoetryFinished": false,
            "isOpen": ((isExtraSmall || isSmall) ? true : false),
            "isNavOpen": false,
            "volNum": 1,
            "step": 0,
            "screenOrientation": "portrait",
            "width": window.innerWidth,
            "height": window.innerHeight
        }



        // this.state = {
        //   totalArticles: [],
        //   savedArticles: {},
        //   // Entire Current Article in Original Format (OF)
        //   entireCurrentArticleOF: [],
        //   currentTitleWordMap: [],
        //   currentAuthorWordMap: [],
        //   currentContentWordMap: [],
        //   isInspiration: false,
        //   isPencilState: false,
        //   isMarkerState: false,
        //   isLoading: true,
        //   isDisplayFromSaved: false,
        //   isPoetryFinished: false,
        //   // isOpen: isMobile ? true : false,
        //   isOpen: false,
        //   isNavOpen: false,
        //   volNum: 1,
        //   step: 0,
        //   screenOrientation: 'portrait',
        // };
    }
    isPortraitMode = () => {
        console.log(this.state);
        const { screenOrientation } = this.state;
        return screenOrientation === 'portrait';
    }

    setScreenOrientation = () => {
        if (window.matchMedia("(orientation: portrait)").matches) {
            console.log('orientation: portrait');
            this.setState({
                screenOrientation: 'portrait'
            });
        }

        if (window.matchMedia("(orientation: landscape)").matches) {
            console.log('orientation: landscape');
            this.setState({
                screenOrientation: 'landscape'
            });
        }
    }

    updateDimensions = () => {
        console.log("Prev dimentions: ", this.state.width, this.state.height)
        this.setState({ width: window.innerWidth, height: window.innerHeight });
        console.log("updating dimentions: ", this.state.width, this.state.height)
    }

    async componentDidMount() {
        window.addEventListener('orientationchange', this.setScreenOrientation);
        // window.addEventListener("resize", this.updateDimensions);

        console.log(this.state);

        // const articles = await getArticles();

        // const pendingContents = articles.map(async (article) => getLexperContent(article.url));
        // const fullContents = await Promise.all(pendingContents);

        // articles.forEach((article, i) => {
        //   article.fullContentText = fullContents[i];
        //   article.id = Uniqid();
        // });

        // this.setState({
        //   totalArticles: articles,
        //   entireCurrentArticleOF: {
        //     ...articles[0],
        //     fullContentText: articles[0].fullContentText,
        //     id: articles[0].id,
        //   },
        //   currentTitleWordMap: createWordMap(articles[0].title),
        //   currentAuthorWordMap: createWordMap(articles[0].author),
        //   currentContentWordMap: createWordMap(articles[0].fullContentText),
        //   isLoading: false,
        // });


    }
    componentWillUnmount() {
        window.removeEventListener('orientationchange', this.setScreenOrientation);
        window.removeEventListener("resize", this.updateDimensions);
    }

    pencilState = () => this.setState({
        isPencilState: true,
        isMarkerState: false,
        isDisplayFromSaved: false,
        isInspiration: false,
        isPoetryFinished: false,
    });

    markerState = () => this.setState({
        isPencilState: false,
        isMarkerState: true,
        isDisplayFromSaved: false,
        isInspiration: false,
        isPoetryFinished: false,
    });

    saveState = () => this.setState({
        isPencilState: false,
        isMarkerState: false,
        isDisplayFromSaved: false,
        isInspiration: false,
        isPoetryFinished: true,
    });

    loadExamples = () => {

        this.setState({
            isInspiration: true,
            isDisplayFromSaved: false,
            isPoetryFinished: false,
        });
    };


    loadNewArticle = async () => {
        const { totalArticles, savedArticles, entireCurrentArticleOF } = this.state;

        let randomNum = getRandomNumberUpTo(10);
        if (!(_.isEmpty(savedArticles))) {
            // console.log(savedArticles, "savedArticles")
            while (totalArticles[randomNum].id in savedArticles) {
                // console.log(totalArticles[randomNum].id, " is in ", savedArticles, "so lets find another number")
                randomNum = getRandomNumberUpTo(10);
            }
        }

        this.setState({
            volNum: randomNum,
            isInspiration: false,
            isMarkerState: false,
            isPencilState: false,
            isDisplayFromSaved: false,
            isPoetryFinished: false,
            entireCurrentArticleOF: {
                ...totalArticles[randomNum],
                fullContentText: totalArticles[randomNum].fullContentText,
                id: totalArticles[randomNum].id,
            },
            currentTitleWordMap: createWordMap(totalArticles[randomNum].title),
            currentAuthorWordMap: createWordMap(totalArticles[randomNum].author),
            currentContentWordMap: createWordMap(
                totalArticles[randomNum].fullContentText,
            ),
        });

    };

    saveCurrentArticle = () => {
        const {
            savedArticles,
            currentTitleWordMap,
            currentContentWordMap,
            currentAuthorWordMap,
            entireCurrentArticleOF,
        } = this.state;

        if (!savedArticles[entireCurrentArticleOF.id]) {
            this.setState({
                savedArticles: {
                    ...savedArticles,
                    [entireCurrentArticleOF.id]: {
                        currentTitleWordMap,
                        currentContentWordMap,
                        currentAuthorWordMap,
                        entireCurrentArticleOF,
                    },
                },
                isInspiration: false,
                isMarkerState: false,
                isPencilState: false,
            });
        } else {
            this.setState({
                savedArticles: {
                    ...savedArticles,
                    [entireCurrentArticleOF.id]: {
                        currentTitleWordMap,
                        currentContentWordMap,
                        currentAuthorWordMap,
                        entireCurrentArticleOF,
                        isInspiration: false,
                        isMarkerState: false,
                        isPencilState: false,
                    },
                },
            });
        }
    };

    onClickHandler = (i, category) => {
        const {
            currentTitleWordMap,
            currentAuthorWordMap,
            currentContentWordMap,
        } = this.state;

        switch (category) {
            case 'title':
                this.setState((prevState) => ({
                    currentTitleWordMap: {
                        ...currentTitleWordMap,
                        [i]: {
                            ...currentTitleWordMap[i],
                            isClicked: !prevState.currentTitleWordMap[i].isClicked,
                        },
                    },
                }));
                break;
            case 'author':
                this.setState((prevState) => ({
                    currentAuthorWordMap: {
                        ...currentAuthorWordMap,
                        [i]: {
                            ...currentAuthorWordMap[i],
                            isClicked: !prevState.currentAuthorWordMap[i].isClicked,
                        },
                    },
                }));
                break;
            case 'content':
                this.setState((prevState) => ({
                    currentContentWordMap: {
                        ...currentContentWordMap,
                        [i]: {
                            ...currentContentWordMap[i],
                            isClicked: !prevState.currentContentWordMap[i].isClicked,
                        },
                    },
                }));
                break;
            default:
                break;
        }
    };

    onMouseOverHandler = (i, category) => {
        const {
            currentTitleWordMap,
            currentAuthorWordMap,
            currentContentWordMap,
        } = this.state;

        switch (category) {
            case 'title':
                this.setState({
                    currentTitleWordMap: {
                        ...currentTitleWordMap,
                        [i]: { ...currentTitleWordMap[i], isMouseOver: true },
                    },
                });
                break;
            case 'author':
                this.setState({
                    currentAuthorWordMap: {
                        ...currentAuthorWordMap,
                        [i]: { ...currentAuthorWordMap[i], isMouseOver: true },
                    },
                });
                break;
            case 'content':
                this.setState({
                    currentContentWordMap: {
                        ...currentContentWordMap,
                        [i]: { ...currentContentWordMap[i], isMouseOver: true },
                    },
                });
                break;
            default:
                break;
        }
    };

    downloadCurrentArticle = () => {
        // MAKE A POST REQUEST TO THE BACKEND => SEND THE NECESSARY DATA
        // ON BACKEND, CREATE A PDF BASED ON A TEMPLATE USING THAT DATA
        // MAKE A GET REQUEST TO GET THAT BLOB BACK FROM THE BACKEND
        // CONVERT A BLOB TO A PDF
        // USE FILE-SAVER TO DOWNLOAD THAT PDF
        // const pdf = null;

        // axios.post('/route', { data })
        //   .then(() => {
        //     axios.get('/route')
        //       .then((response) => {
        //         pdf = response.data;
        //       });
        //   });
    }

    onSaveHandler = (i) => {
        const { savedArticles } = this.state;

        const {
            currentAuthorWordMap: clickedAuthorWordMap,
            currentTitleWordMap: clickedTitleWordMap,
            currentContentWordMap: clickedContentWordMap,
        } = savedArticles[i];

        this.setState({
            step: 1,
            isDisplayFromSaved: true,
            isPoetryFinished: false,
            isInspiration: false,
            currentAuthorWordMap: clickedAuthorWordMap,
            currentTitleWordMap: clickedTitleWordMap,
            currentContentWordMap: clickedContentWordMap,
        });
    };

    deleteSavedHandler = (i) => {
        const { savedArticles } = this.state;
        const updatedSavedArticles = _.omit(savedArticles, i);
        this.setState({ savedArticles: { ...updatedSavedArticles } });
    };

    handleOpen = () => {
        this.setState({ isOpen: true });
    };

    handleClose = () => this.setState({ isOpen: false });

    toggleNav = () => this.setState((prevState) => ({ isNavOpen: !prevState.isNavOpen }));

    render() {
        const {
            entireCurrentArticleOF,
            savedArticles,
            isInspiration,
            currentTitleWordMap,
            currentAuthorWordMap,
            currentContentWordMap,
            volNum,
            isPoetryFinished,
            isDisplayFromSaved,
            inspirationImg,
            isPencilState,
            isMarkerState,
            isLoading,
            isNavOpen,
            isOpen,
            step,
        } = this.state;

        const displayComponent = (i) => {
            switch (i) {
                // mobile view instruction
                case 0:

                    this.setState({ step: 0 });
                    this.updateDimensions();
                    console.log("MOBILE")
                    break;
                // newspaper view
                case 1:

                    this.setState({ step: 1 });
                    this.updateDimensions();
                    console.log("NEWSPAPER")

                    break;
                // saved Newspaper
                case 2:
                    this.updateDimensions();
                    console.log("SAVED")

                    this.setState({ step: 2 });
                    break;
                default:
                    break;
            }
        };

        const componentToBeRendered = () => {
            switch (step) {
                case 0:
                    return (
                        <>
                            <MobileLanding handleOpen={this.handleOpen} />
                            <Modal handleClose={this.handleClose} isOpen={isOpen} />
                        </>
                    );
                case 1:
                    return (
                        <Grid item sm={12} md={8} className={styles['newspaper-container']}>
                            <Newspaper
                                volNum={volNum}
                                entireCurrentArticleOF={entireCurrentArticleOF}
                                isPencilState={isPencilState}
                                isMarkerState={isMarkerState}
                                isInspiration={isInspiration}
                                isDisplayFromSaved={isDisplayFromSaved}
                                inspirationImg={inspirationImg}
                                currentTitleWordMap={currentTitleWordMap}
                                currentAuthorWordMap={currentAuthorWordMap}
                                isPoetryFinished={isPoetryFinished}
                                currentContentWordMap={currentContentWordMap}
                                onClickHandler={this.onClickHandler}
                                onMouseOverHandler={this.onMouseOverHandler}
                                loadNewArticle={this.loadNewArticle}
                                loadExamples={this.loadExamples}
                                pencilState={this.pencilState}
                                markerState={this.markerState}
                                saveState={this.saveState}
                                saveCurrentArticle={this.saveCurrentArticle}
                            />
                        </Grid>
                    );
                case 2:
                    return (
                        <Grid
                            item
                            xs={12}
                            md={2}

                            className={styles['saved-newspapers-container']}
                        >
                            <SavedNewspaper
                                onSaveHandler={this.onSaveHandler}
                                savedArticles={savedArticles}
                                volNum={volNum}
                                deleteSavedHandler={this.deleteSavedHandler}
                                isPoetryFinished={isPoetryFinished}
                            />
                        </Grid>
                    );
                default:
                    return null;
            }
        };

        if (_.isEmpty(entireCurrentArticleOF)) {
            return (
                <Grid container className={styles['main-container']}>
                    <Loader
                        loaded={false}
                        scale={2.0}
                        top="50%"
                        left="50%"
                        position="relative"
                        loadedClassName={styles.loader}
                    />
                </Grid>
            );
        }

        if (isMobile) {
            return (
                <>
                    <Button
                        type="button"
                        className={styles['nav-container']}
                        onClick={this.toggleNav}
                    >
                        <DehazeIcon />
                    </Button>
                    <MobileNav
                        toggleNav={this.toggleNav}
                        isNavOpen={isNavOpen}
                        displayComponent={displayComponent}
                    />
                    {
                        componentToBeRendered() || <Modal handleClose={this.handleClose} isOpen={isOpen} />
                    }
                </>
            );
        }

        return (
            <Grid container alignContent='center' justify='center' className={styles['main-container']}>
                <Modal handleClose={this.handleClose} isOpen={isOpen} />
                {console.log(`orientation: from render: isPortraitMode = ${this.isPortraitMode()}`)}
                <Grid item className={styles['bg-showing']} md={2} lg={2}>
                </Grid>
                <Grid
                    item
                    xs={12}
                    md={8}
                    lg={7}
                    className={classNames(styles['newspaper-container'], isLoading ? styles.loader : '')}
                >
                    <Newspaper
                        volNum={volNum}
                        entireCurrentArticleOF={entireCurrentArticleOF}
                        isPencilState={isPencilState}
                        isMarkerState={isMarkerState}
                        isInspiration={isInspiration}
                        isDisplayFromSaved={isDisplayFromSaved}
                        isPoetryFinished={isPoetryFinished}
                        inspirationImg={inspirationImg}
                        currentTitleWordMap={currentTitleWordMap}
                        currentAuthorWordMap={currentAuthorWordMap}
                        currentContentWordMap={currentContentWordMap}
                        onClickHandler={this.onClickHandler}
                        onMouseOverHandler={this.onMouseOverHandler}
                        loadNewArticle={this.loadNewArticle}
                        loadExamples={this.loadExamples}
                        pencilState={this.pencilState}
                        markerState={this.markerState}
                        saveState={this.saveState}
                        saveCurrentArticle={this.saveCurrentArticle}
                        handleOpen={this.handleOpen}
                    />
                </Grid>
                <Grid item xs={12} md={2} lg={3} className={styles['saved-np']}>
                    <SavedNewspaper
                        onSaveHandler={this.onSaveHandler}
                        savedArticles={savedArticles}
                        volNum={volNum}
                        deleteSavedHandler={this.deleteSavedHandler}
                        isPoetryFinished={isPoetryFinished}
                    />
                </Grid>
            </Grid >
        );
    }
}

export default App;

-----------------
    this.state = {
    "totalArticles": [
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "Brendan Hesse",
            "title": "Here's How Much You'll Save With an Apple-Refurbished iPhone XS",
            "description": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS…",
            "url": "https://lifehacker.com/heres-how-much-youll-save-with-an-apple-refurbished-iph-1841138323",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/roo6mjgthvj1jtb5ayr1.jpg",
            "publishedAt": "2020-01-21T21:00:00Z",
            "content": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS… [+2300 chars]",
            "fullContentText": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones  through its online store  and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS Max (or or make the full jump to an iPhone 11). The XS and XS Max are barely two years old, and their tech is still plenty fast, so they’ll still be solid upgrades.\nThe real question, of course, is how much can you save on a refurbished iPhone XS or XS Max you buy directly from Apple. Exact final pricing depends on the model and configuration you go for, but all iPhone XS models appear to be $200 cheaper than buying new, while you’ll save around $250 for an iPhone XS Max. The highest prices you can expect to pay for the 512GB models of each phone (sans tax) are:\n-  512GB iPhone XS:  $999\n-  512GB iPhone XS Max:  $1,099\n\n(Note: all refurbished models sold by Apple are unlocked).\nFor reference, here’s a quick comparison of those same refurbished 512GB unlocked iPhone XS and XS Max models from popular third-party aftermarket sellers (these prices are based on the current stock at time of writing):\nSwappa\n-  iPhone XS:  $659\n-  iPhone XS Max:  $945\n\nBackmarket\n-  iPhone XS:  $799-$999\n-  iPhone XS Max:  $779\n\nGazelle\n-  iPhone XS:  $650\n-  iPhone XS Max:  $750\n\nRefurb.me\n-  iPhone XS:  $649-$999\n-  iPhone XS Max:  $749-$1099\n\nAs you can see, Apple’s prices are higher than other sellers, but buying from a third-party seller can be a gamble. Unless you’ve bought from a site or seller before, you can’t be sure the device runs as advertised. Some sellers offer warranties or trade-in programs should you wind up with a bad purchase, but it’s not a guarantee.\nConversely, all of Apple’s refurbished devices are  cleaned and tested by Apple  to ensure they meet the company’s quality standards. They’re not going to be as flawless as a brand-new device, but it can be pretty close. You’ll also get a one-year warranty on all devices and can extend coverage through AppleCare+.\nTo be clear, we’re not saying that you should only buy a refurbished iPhone XS or XS Max from Apple, but there are certain benefits Apple’s program guarantees its customers that can make buying one of their refurbished devices less risky, even with the higher prices. If you can find a better deal you trust elsewhere, then go for it. If not, Apple’s store is a decent option if you’re in the market for a slightly cheaper iPhone XS.",
            "id": "k69plbdg"
        },
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "Abu Zafar",
            "title": "How to Take a Screenshot on iPhone X, XS and 11",
            "description": "If you’ve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhon…",
            "url": "https://lifehacker.com/how-to-take-a-screenshot-on-iphone-x-xs-and-11-1841064538",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/jz1kqcyhtcozukg1wike.jpg",
            "publishedAt": "2020-01-21T14:30:00Z",
            "content": "If youve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhone… [+330 chars]",
            "fullContentText": "If you’ve finally upgraded that old iPhone 6 to a brand new iPhone X or 11, you might be wondering how to take a screenshot. Previously, all it took was a simultaneous press of the side button and home button. But with the loss of the home button in the iPhone X redesign in 2017, the button combination has changed. In the short video above, I show you two ways to take a screenshot on a modern iPhone.\nQuick Fix is a video series where I tackle your most commonly asked tech questions in 90 seconds or less. If you have a question you want me to answer, leave a comment below or email me.",
            "id": "k69plbdh"
        },
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "David Murphy",
            "title": "You Can't Stream 'Star Trek: Picard' on Linux Without This Workaround",
            "description": "It’s 2020, and you think that it would be easy to stream any content you want on any device you want—especially that brand-new Picard series on CBS All Access. If you’re an ubergeek who loves both Star Trek and Linux, you’re out of luck. Thanks to the joys of…",
            "url": "https://lifehacker.com/you-cant-stream-star-trek-picard-on-linux-without-this-1841327097",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/v4nesxfyduqgoy7qpdct.png",
            "publishedAt": "2020-01-29T19:00:00Z",
            "content": "Its 2020, and you think that it would be easy to stream any content you want on any device you wantespecially that brand-new Picard series on CBS All Access. If youre an ubergeek who loves both Star Trek and Linux, youre out of luck. Thanks to the joys of dig… [+2005 chars]",
            "fullContentText": "It’s 2020, and you think that it would be easy to stream any content you want on any device you want—especially that brand-new  Picard  series on CBS All Access. If you’re an ubergeek who loves both  Star Trek  and Linux, you’re out of luck. Thanks to the joys of digital rights management, or DRM, you’re unable to use CBS’ All Access service to stream Picard on Linux.\nIn a cruel twist, not only can you not stream the show, but you  can  stream all the bullshit that comes wrapped around it. As Ars Technica’s Jim Salter  describes  :\n“Moving back to one of my own PCs, I was quickly able to confirm the issue: trailers autoplay properly, and even the ads work—but the actual content won’t play on a Linux desktop PC on any browser including Google Chrome. Diving into the Chrome Web Console, we can see HTTP 400 (Bad Request) errors when the browser attempts to fetch a license from CBS’  Widevine  back end.”\nThe more tech-savvy among you are probably assuming that you can just change your browser’s user agent to fool CBS All Access into thinking you’re using a supported platform. Not so. (Salter tried that, too.) For whatever reason, Widevine and CBS All Access don’t play nice with Linux, even though other streaming services that use Widevine have no issues.\nThe only workaround to this vexing problem, aside from watching CBS All Access on a different device entirely, is to cancel your regular CBS All Access subscription and instead subscribe to the service via  Amazon Prime Video  . Yes, you’ll need to be an existing Amazon Prime member for this to work (in the U.S., at least), which will already set you back $120 annually. After that, adding CBS All Access costs the same as if you were subscribing standalone: $6/mo for limited commercials or $10/mo for commercial-free streaming.\nThat is, however, slightly more than you’d pay if you signed up for an annual plan  via CBS itself  ($60/year for limited commercials and and $100/year for no commercials). Amazon doesn’t offer these options, so this route is best if you’re only planning to binge a show or two and then cancel your service. If you’re looking to stick with CBS All Access for the long haul, you might just want to try not watching it on Linux; a tablet, phone, or app on your Smart TV will save you more cash.",
            "id": "k69plbdi"
        },
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "Josh Ocampo",
            "title": "Why You Shouldn't Let Celebrities Text You Via Community",
            "description": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on socia…",
            "url": "https://lifehacker.com/why-you-shouldnt-let-celebrities-text-you-via-community-1841259489",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/sgh5mxng1qcls0vqmmp6.jpg",
            "publishedAt": "2020-01-27T18:30:00Z",
            "content": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on socia… [+1776 chars]",
            "fullContentText": "Targeted ads, the kind that might appear on your Facebook feed or Twitter timeline, can feel awfully invasive; after all, they often appear using personal information like your location, age or search history. And thanks to a new service making waves on social media, they’re likely about to target you even more closely.\nRecently, if you’ve ever stumbled across a tweet or Instagram in which a celebrity posts their “phone number” so that they can interact with and text you directly, it’s not what you think—instead of entering into a personal chat with a celebrity, you’ve  actually  subscribed to a newsletter of automated updates that’ll be sent by text.\nBack in October, reporter Taylor Lorenz described how it works in detail for the  New York Times  . If you “text” a celebrity using this number, you’re likely signing up for a service run by a tech-startup called  Community  ; they help manage direct messaging with large audiences on behalf of celebrities and other well-known figures, as a way of “monetizing” their social network.\nOnce you opt-in by way of text, what kind of updates might you receive? Well, it depends. Sometimes, it’s just a random string of photos and texts, as Lorenz received from actor Ashton Kutcher. I signed up for  Pod Save America’s  Jon Favreau’s updates and just received a bunch of updates about their tour.\nIf you’re really, really lucky, you  could  a text back from the celebrity in question—but don’t count on it. Given the number of messages they likely receive, it would seem like a huge effort just to respond to every text. The vast majority of the time, you’re likely only at the receiving end of these automated updates.\nIf you’re a particular fan of a celebrity, maybe signing up isn’t such a bad idea. But we’ll do you a favor and tell you now that it probably isn’t worth it. The odds are incredibly low that you’ll receive a text that isn’t just an automated update, and while  Community ensures  that they will not sell your personal details to other third parties, well,  we’ve all heard that before  , haven’t we?",
            "id": "k69plbdj"
        },
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "David Murphy",
            "title": "Help! I Bumped My PC and Now It Won't Turn On",
            "description": "Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could…",
            "url": "https://lifehacker.com/help-i-bumped-my-pc-and-now-it-wont-turn-on-1841360774",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/o2oyoxjrp7m2htuheipk.jpg",
            "publishedAt": "2020-01-31T14:00:00Z",
            "content": "Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could… [+5747 chars]",
            "fullContentText": "Tech 911  Do you have a tech question keeping you up at night? We'd love to answer it! Email david.murphy@lifehacker.com with \"Tech 911\" in the subject line.  Prev  Next Your laptop computer is a delicate beast. It can take a pounding, perhaps even a dropping, up to a certain point. It might survive a fall from the desk without a scratch, but bumping it with something heavy in just the right way, at just the right time, could end it all.\nYour desktop PC is a bit more protected, in theory, since it probably lives in a bulky case. That doesn’t mean it’s invulnerable. Though we weren’t sent this question by a Lifehacker reader, as is custom for the  Tech 911  column, I stumbled across this call for help on Reddit and couldn’t resist highlighting it in this this week’s edition. It’s just that good.\nRedditor  u/dyingquickly  describes  :\n“I lightly bumped my computer it shot off and makes no attempt to boot when I hit the power button. I know my PSU is working because I have a tester. I’ve triple checked all my connections. Does anyone know what the problem might be? The light of the on button doesn’t even appear, it’s as if the button isn’t working but i don’t know what the problem is.”\nGenerally speaking, when your computer zaps off like that, it’s a pretty good indication that you’re going to have to do some troubleshooting. If you’re lucky, you’re only dealing with a random power outage or loose cable—possibly even a button on your surge protector that you brushed up against. You should be able to correct the problem and turn your system back on again, suffer a quick disk scan, and resume whatever it is you were doing. (Here’s hoping your work or  browser tabs  autosaved.)\nWhen your system  doesn’t  turn on is when this process really gets fun.\n\nCHECK FOR LOOSE CABLES\nSuppose you bang into your desktop PC with your chair, your foot, or your stupid but loveable pet runs into it. We’ll classify this as a “hard” bump, not a gentle dusting. If your desktop PC won’t turn back on—as in, it won’t even POST—my first assumption is a crucial cable has fallen out of a connector  just enough  to cause an issue.\nThis is going to be annoying, but I recommend popping open your desktop PC and making sure everything looks right. If some cable isn’t connected in some awkward way, that might be the culprit—though your motherboard’s primary power cables  should  have locking mechanisms that make this impossible.  I wouldn’t rule anything out  . In fact, I’d start with them: the fat 24-pin cable and the smaller 4/8-pin cable.\n\nSTART DISCONNECTING AND RECONNECTING CRITICAL CABLES\nIf a loose cable isn’t immediately apparent, you might want to take the more annoying step of unplugging everything that’s plugged in and reconnecting it. I’d start with more critical cables first—those going into your motherboard, graphics card, and modular power supply (if applicable). You can try your SATA cables, too, as well as the power connections for your hard drive(s). (Though your motherboard should still be able to POST without a hard drive connected, let’s not take any chances.)\nYou’ll also want to check your system’s tiny front-panel connectors. While a loose front-panel cable would explain why you can’t turn your system back on, you’ll also likely be able to surmise this is the issue if your motherboard is otherwise powered on (via any lights you see). If your motherboard has its own power button you can use to start your system, and pressing that makes everything work correctly, then you’re likely dealing with some issue with your front-panel cables.\nAlso, make sure you’re checking the rear of your system. There are plenty of cables back there—the power cable, especially—so it’s possible that something got jiggered out of place in such a way as to prevent your computer from turning on. Again, this might feel like try-anything troubleshooting, but I’m willing to bet that the answer to this issue is going to be some random cable you wouldn’t have normally thought about in your troubleshooting. Heck, you can even try resetting the CMOS battery; I doubt it’s the culprit, but it’s worth exploring if your other ideas don’t pan out.\nIt’s also possible that you bumped something out of place in your case that is now causing some kind of short-circuit with your motherboard. That could be a cable; that could be a screw that you accidentally left in your system when you were building it. However, short of eyeballing the problem, taking your system apart and rebuilding it is likely going to be the best way to address this. Given how long that might take to do, I’d save it as a final option.\nAnd if you go this route, be  methodical  . Strip your system to the bare components. Take the CPU and cooler off your motherboard, remove the RAM, and unplug everything (to name a few steps). Once you’re staring at a bunch of components, plug everything back in as if you’re  building a computer for the first time  . Connect the basics—your motherboard, your RAM, and your video card (if applicable)—and try powering on your system. Can you make it into the BIOS? If so, power down your PC and start adding your other hardware one at a time, until you can deduce what, if anything, is causing issues.\n\nTRY FIGURING OUT WHICH COMPONENT HAS FAILED\nAnd if you still can’t power on your PC, you might want to phone a friend. Slap your hard drive into their system or external enclosure to confirm your data is safe (and back it up, if need be)? See if you can borrow an extra power supply to use with your core components. Do they have any extra RAM you could try? Will your video card work on their system?\nAt this point, you’re likely trying to pinpoint faulty hardware, since odds are getting higher that you actually did some kind of damage to something inside your system. Hopefully you don’t have to reach this point; I’ve been there, and it’s not a fun bit of detective work.\nDo you have a tech question keeping you up at night? Tired of troubleshooting your Windows or Mac? Looking for advice on apps, browser extensions, or utilities to accomplish a particular task? Let us know! Tell us in the comments below or email  .",
            "id": "k69plbdk"
        },
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "David Murphy",
            "title": "Change These Facebook Settings to Protect Your Photos From Facial Recognition Software",
            "description": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesn’t make this weekend’s report from The New York Times any less dam…",
            "url": "https://lifehacker.com/change-these-facebook-settings-to-protect-your-photos-f-1841131425",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/kgodep1gxupxb0lqj9je.png",
            "publishedAt": "2020-01-21T20:00:00Z",
            "content": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesnt make this weekends report from The New York Times any less damni… [+4316 chars]",
            "fullContentText": "It should come as little surprise that any content you offer to the web for public consumption has the potential to be scraped and misused by anyone clever enough to do it. And while that doesn’t make this weekend’s report from  The New York Times  any less damning, it’s a great reminder about how important it is to  really  go through the settings for your various social networks and limit how your content is, or can be, accessed by anyone.\nI won’t get too deep into the  Times’  report; it’s worth reading on its own, since it involves a company (Clearview AI) scraping more than three billion images from millions of websites, including Facebook, and creating a facial-recognition app that does a pretty solid job of identifying people using images from this massive database.\nEven though Clearview’s scraping techniques technically violate the terms of service on a number of websites, that hasn’t stopped the company from acquiring images  en masse.  And it keeps whatever it finds, which means that turning all your online data private isn’t going to help if Clearview has already scanned and grabbed your photos.\nStill, something is better than nothing. On  Facebook  , likely the largest stash of your images, you’re going to want to visit  Settings >  Privacy  and look for the option described:  “Do you want search engines outside of Facebook to link to your profile?”\nTurn that off, and Clearview won’t be able to grab your images. That’s not the setting I would have expected to use, I confess, which makes me want to go through all of my social networks and rethink how the information I share with them flows out to the greater web.\n\nLOCK DOWN YOUR FACEBOOK EVEN MORE WITH THESE SETTINGS\nSince we’re already here, it’s worth spending a few minutes wading through Facebook’s settings and making sure as much of your content is set to friends-only as possible. That includes changing  “Who can see your future posts”  to “friends,” using the  “Limit Past Posts”  option to change everything you’ve previously posted to friends-only, and making sure that only you can see your friends list—to prevent any potential scraping and linking that some third-party might attempt. Similarly, make sure only your friends (or friends of friends) can look you up via your email address or phone number. (You never know!)\nYou should then visit the  “  Timeline and Tagging  ”  settings page and make a few more changes. That includes only allowing friends to see what other people post on your timeline, as well as posts you’re tagged in. And because I’m a bit sensitive about all the crap people tag me in on Facebook, I’d turn on the “Review” options, too. That won’t help your account from being scraped, but it’s a great way to exert more control over your timeline.\nFinally, even though it also doesn’t prevent companies from scraping your account, pull up the  “  Public posts  ”  section of Facebook’s settings page and limit who is allowed to follow you (if you desire). You should also restrict who can comment or like your public information, like posts or other details about your life you share openly on the service.\n\nONCE I FIX FACEBOOK, THEN WHAT?\nHere’s the annoying part. Were I you, I’d take an afternoon or evening and write out all the different places I typically share snippets of my life online. For most, maybe that’s probably a handful of social services: Facebook, Instagram, Twitter, YouTube, Flickr, et cetera.\nOnce you’ve created your list, I’d dig deep into the settings of each service and see what options you have, if any, for limiting the availability of your content. This might run contrary to how you use the service—if you’re trying to gain lots of Instagram followers, for example, locking your profile to “private” and requiring potential followers to request access might slow your attempts to become the next big Insta-star. However, it should also prevent anyone with a crafty scraping utility to mass-download your photos (and associate them with you, either through some fancy facial-recognition tech, or by linking them to your account).\nWill you be able to fully prevent someone from finding a way to build the next big database of searchable images? Probably not. But the most important lesson we can take from situations like these—Clearview AI’s...well...existence—is that you should understand the nuances of the services you share your information to, and you should always use your settings to restrict access to your data in whatever way you feel most comfortable. Were I you, I’d err on the side of “let trusted people see what I do instead of anyone on the internet,” but that’s just me.",
            "id": "k69plbdl"
        },
        {
            "source": {
                "id": null,
                "name": "Lifehacker.com"
            },
            "author": "David Murphy",
            "title": "I Lost Nine Years of Photos by Locking Myself Out of My Google Account",
            "description": "I normally use this weekly column to answer people’s technology-themed questions. This week, I’m taking a slight departure, because I think sharing a reader’s story is important—even though there’s not really much I, nor Google, can do in the case of her lock…",
            "url": "https://lifehacker.com/i-lost-nine-years-of-photos-by-locking-myself-out-of-my-1841178170",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/qsqfi9lclqdlo3adj7ih.jpg",
            "publishedAt": "2020-01-24T14:00:00Z",
            "content": "I normally use this weekly column to answer peoples technology-themed questions. This week, Im taking a slight departure, because I think sharing a readers story is importanteven though theres not really much I, nor Google, can do in the case of her locked Go… [+6981 chars]",
            "fullContentText": "Tech 911  Do you have a tech question keeping you up at night? We'd love to answer it! Email david.murphy@lifehacker.com with \"Tech 911\" in the subject line.  Next I normally use  this weekly column  to answer people’s technology-themed questions. This week, I’m taking a slight departure, because I think sharing a reader’s story is important—even though there’s not really much I, nor Google, can do in the case of her locked Google account. There’s a lot we can learn from her example, and a few items you should check to make sure this frustrating issue never happens to you.\nLifehacker reader  Cathryn  writes:\nI stupidly forgot my Google account password back in November 2019, I had only just changed it. I asked them to email to text me a verification code, like I’d done so many times before, however I stupidly added the 2 step verification on my account.\nThe problem was I inputted an incorrect recovery email address, I missed 2 digits off my email. Therefore the 2 step verification wouldn’t work. Google locked my account, I also broke my mobile phone screen days later that I ended up buying a new handset but the same mobile phone number.\n[...]\nI only get asked 1 security question “what was your first phone number” I picked that question myself in the security settings a long time ago. Then I get asked to give them an email they can contact me on, then a few days later I receive an email stating they don’t have enough evidence so unfortunately they can’t open my account.\nI never get asked any questions such as when was your account created. I’ve tried recovery options from my laptop, kindle ect that used the same gmail account plus it’s all from my home IP address.\nI get it’s my fault for initially forgetting my password, but google security settings are flawed as when I put a recovery email they should send an automatic email confirmation to make sure it’s correct due to human error. I’ve lost lots of important document and 9 years of Google photos with some pictures of loved ones that I’ll never get back.\nI present this reader story as a cautionary tale. First, whenever you’re dealing with two-factor authentication or any kind of account-recovery options—whether you’re inputting your email or phone number to set it up, or you’re copying down precious backup codes—it’s critical that you triple-check this information. That’s not because there’s no way to change it; you can obviously change details like your email address or phone number if you have access to your account. However, you’re less likely to this because most people assume that everything is on the up and up once they’ve set up their accounts. And if it isn’t, and you left a typo in a critical account recovery option, you’re going to have a mess of a time when you try to restore access.\nGoogle makes this entire situation particularly frustrating because they, a giant search company that practically prints money, have no desire to set up a customer service center that Google users can call whenever they have issues with Google’s services. Regular (free) Google users get a  Help Center  and some community forums, but that’s it.\nAnd while I’m a little snarky about this, I do see Google’s point of view. First off, most people aren’t paying Google a dime for the services they’re using. Yes, Google is doing everything it can to extract data from you so it can better target you with relevant advertising, but most people aren’t coughing up cash for the privilege of using Gmail, for example. You might pay Google a little per month  for some extra storage  , but that’s about it. Why dedicate a not-so-insignificant amount of resources to a service center for free services?\nSecond, and more importantly, Google’s lack of one-to-one help for issues like account recovery is actually a security measure in itself. Consider the alternative: If someone sort-of knew your email address, phone number, and/or mailing address, in combination with some information they’ve pilfered from one of the many data breaches out there, would you want them to be able to convince a Google customer service representative that they are actually you?\nGiving everyone the same, generic account recovery process, and letting nobody have access to any special favors or extra support, makes it harder to get into a locked account when you have unique issues, sure. It also makes it harder for others to get into  your  account. I’m not saying I’m a fan of this trade-off—because real people  do  have complex issues that need special help—but it does help limit an unscrupulous person’s ability to social engineering their way into your Google account.\nThis puts the burden on you to make sure that all of your recovery mechanisms for your Google account are accurate. Even if you’ve already set them up and are feeling pretty good about it, I recommend taking a quick few minutes to double-check. It never hurts, because you don’t want to deal with the alternative of not being able to access your account in the event of some major mess or security breach.\nTo get started, visit the  Security  section of your Google account settings. Scroll down a bit to the “Ways we can verify it’s you” list of questions and make sure your phone number, recovery email address, and security question are all accurate.\nNext, scroll up a smidge and click on  “  2-Step Verification  .”  You’ll have to enter your password again. On the subsequent screen, make sure that you’ve turned  on  Google’s two-step verification to better protect your account. Assuming you have, scroll down a bit and click on “Show Codes” under the “Backup codes” section.\nCopy. These. Codes. Down.  To paraphrase Gandalf, keep them secret and safe; you’ll use them to log back into your account if, for whatever reason, you can’t access the device(s) you’ve set up for two-step verification.\nFinally, go visit the  Personal info  section of your Google account settings and double-check that your actual contact information is correct: email(s) and phone number(s). While you’re here, you might as well also make sure you’ve entered your correct birthday, too.\nAnd, of course, the best way to make sure you never forget your passwords to anything is to use  a strong password manager  . Even if it costs you a few bucks a month, it’s worth every penny. Not only does this make it easy to keep track of your passwords, but you can use stronger, more secure passwords that are  very  difficult to guess or brute-force. Also, a great password manager will let you know whenever your saved accounts are compromised in a data breach (which should be less of an issue if you’re using unique passwords for each service, but people can get a little lazy about this).\nThat’s my advice to everyone, and it applies to more than just Google. If a service is important to you, make sure you know the process of how you’ll get back into your account if you’re ever locked out. Then, check your data to make sure any errant typos won’t come back to haunt you, and make sure you’ve saved any backup codes or any other information you need in order to regain access to a lost or locked account. You’ll be glad you did now, if (or when) you have to deal with this later.\nDo you have a tech question keeping you up at night? Tired of troubleshooting your Windows or Mac? Looking for advice on apps, browser extensions, or utilities to accomplish a particular task? Let us know! Tell us in the comments below or email  .",
            "id": "k69plbdm"
        },
        {
            "source": {
                "id": null,
                "name": "Theinventory.com"
            },
            "author": "Tercius, Ana Suarez, and Corey Foster on Kinja Deals, shared by Ana Suarez to Lifehacker",
            "title": "Monday's Best Deals: Sony ANC Headphones, Tile Gold Box, Witcher 3: Wild Hunt, and More",
            "description": "A windshield wiper sale, 2TB Seagate Firecuda, foam roller, and a supplement Gold Box lead of Monday’s best deals from around the web. Read more...",
            "url": "https://kinjadeals.theinventory.com/mondays-best-deals-sony-anc-headphones-tile-gold-box-1841258693",
            "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/fkz6rbhgodm1ezkghyji.jpg",
            "publishedAt": "2020-01-27T15:30:00Z",
            "content": "A windshield wiper sale, 2TB Seagate Firecuda, foam roller, and a supplement Gold Box lead of Mondays best deals from around the web.\r\nBookmark Kinja Deals and follow us on Twitter to never miss a deal.\r\neBay is offering one of the best deals on the best nois… [+14280 chars]",
            "fullContentText": {},
            "id": "k69plbdn"
        },
        {
            "source": {
                "id": "engadget",
                "name": "Engadget"
            },
            "author": "Richard Lawler",
            "title": "The Morning After: Meet Cadillac's tech-heavy new Escalade",
            "description": "Hey, good morning! You look fabulous. Good morning! Since there's nothing at all wonky to report with Election 2020 results and incredibly flawed apps that were shoddily included in the primary process, let me point you to your next 4K demo reel. It's from 18…",
            "url": "https://www.engadget.com/2020/02/05/2021-escalade-morning-after/",
            "urlToImage": "https://o.aolcdn.com/images/dims?thumbnail=1200%2C630&quality=80&image_uri=https%3A%2F%2Fo.aolcdn.com%2Fimages%2Fdims%3Fresize%3D2000%252C2000%252Cshrink%26image_uri%3Dhttps%253A%252F%252Fs.yimg.com%252Fos%252Fcreatr-uploaded-images%252F2020-02%252F73cf1810-4810-11ea-bfbb-f06a233254c6%26client%3Da1acac3e1b3290917d92%26signature%3D3fa8c987b941255de22db64185ddf92abbe04f9b&client=amp-blogside-v2&signature=303b1045fccfa43bdb58bee9ac63b38f8b8e5057",
            "publishedAt": "2020-02-05T12:30:00Z",
            "content": "Games are capped at 1080p and 60 fps.NVIDIA's GeForce Now cloud gaming service is live\r\nAfter seven years of tweaking its delivery systems and gathering beta feedback, NVIDIA has finally unveiled the consumer version of its game-streaming ecosystem. GeForce N… [+3181 chars]",
            "fullContentText": "Games are capped at 1080p and 60 fps.  NVIDIA's GeForce Now cloud gaming service is live\nAfter seven years of tweaking its delivery systems and gathering beta feedback, NVIDIA has  finally unveiled the consumer version of its game-streaming ecosystem  . GeForce Now streams games of all sizes to PC, Mac, Android and the NVIDIA Shield, and it works with players' existing libraries on Steam, the Epic Games Store and all other digital platforms.\nA free option allows folks to connect to NVIDIA's servers and play for one hour at a time. After an hour, these players will need to reconnect and potentially wait for a spot to open up on the servers. In the paid tier, players go to the front of the line when logging on, they'll stay online for up to six hours and get access to RTX hardware.\nAnd release dates for some highly anticipated shows.  Disney+ has 28.6 million subscribers so far\nIn Disney's first earnings report since the launch of Disney+, the company announced  how many people have signed up for its streaming service  . As of Monday, the number is 28.6 million, which CEO Bob Iger said he doesn't expect to change significantly until this fall. That's when October's  The Mandalorian  season two arrives, as well as MCU-spinoff shows  Falcon and the Winter Soldier  (August) and  Wandavision  (December).\nIt's OK, though, they apologized.  Google Photos bug sent private videos to strangers\nIf you used Google Takeout to back up your Photos files from November 21st to 25th, 2019, a stranger out there may have a copy of your videos. During that period, the email reads, \"some videos in Google Photos were incorrectly exported to unrelated users' archives.\" That means  someone out there may have a copy of your (hopefully wholesome) videos  , and you may have a copy of someone else's, as well.\nDon't forget the augmented reality navigation system.  2021 Cadillac Escalade packs 38 inches of curved OLED screens and Super Cruise\nCadillac is introducing a new Escalade later this year, and inside you'll find three curved OLED screens in the dashboard. There's a 7.2-inch touch panel to the driver's left, a 14.2-inch screen info cluster behind the steering wheel and the 16.9-inch infotainment system. With \"perfect\" black levels and bright colors, the screens don't need hoods over them to block out the light.\nThe luxury SUV line also packs five USB ports (three of them are USB-C), wireless charging, an optional 36-speaker audio system that uses mics to let front and rear passengers communicate, Super Cruise driving assist that can now change lanes by itself, NFC and electric motors that help the doors close quietly.  Check out all the details and get a closer look here  .\nThe show is scheduled to start February 24th.  LG pulls out of Mobile World Congress over coronavirus concerns\nAs concerns over a growing coronavirus outbreak continue to mount, LG has decided that the Barcelona-based trade show  simply isn't worth the risk  . Instead, LG will hold separate events in the \"near future\" to reveal its batch of 2020 smartphones, which is expected to include a sequel to last year's ambitious, underwhelming G8 ThinQ.\n\nBUT WAIT, THERE'S MORE...\n-  Iowa Democrats say sloppy app code delayed caucus results\n-  Sony sets prices for (most of) its 2020 4K LED TVs\n-  'Altered Carbon' season two teaser shows Anthony Mackie as the 'new' hero\n-  Now T-Mobile and Sprint can verify calls across their networks to prevent robocalls\n-  KFC slips ads into Spotify Premium through artist profiles\n-  Dyson patents a wearable air purifier that doubles as headphones\n-  SpaceX asks permission to take Starship on a high-altitude test flight\n-  Rockstar Games co-founder Dan Houser is leaving the company\n\nThe Morning After is a new daily newsletter from Engadget designed to help you fight off FOMO. Who knows what you'll miss if you don't  Subscribe  .\nCraving even more?  Like us on Facebook  or  Follow us on Twitter  .\nHave a suggestion on how we can improve The Morning After?  Send us a note  .\nIn this article:  av  ,  business  ,  design  ,  entertainment  ,  gadgetry  ,  gadgets  ,  gear  ,  internet  ,  mobile  ,  space  ,  themorningafter  ,  transportation  All products recommended by Engadget are selected by our editorial team, independent of our parent company. Some of our stories include affiliate links. If you buy something through one of these links, we may earn an affiliate commission.  Comments",
            "id": "k69plbdo"
        },
        {
            "source": {
                "id": "engadget",
                "name": "Engadget"
            },
            "author": "Rachel England",
            "title": "Yubico is making it easier for businesses to buy its YubiKeys",
            "description": "A growing number of companies are looking at hardware authentication security keys as a trusted and convenient way to protect sensitive corporate data. Indeed, Google has recently launched an open source project to help advance the uptake of this technology. …",
            "url": "https://www.engadget.com/2020/02/04/yubico-business-yubikey-enterprise-security-key/",
            "urlToImage": "https://o.aolcdn.com/images/dims?thumbnail=1200%2C630&quality=80&image_uri=https%3A%2F%2Fo.aolcdn.com%2Fimages%2Fdims%3Fcrop%3D1000%252C656%252C0%252C147%26quality%3D85%26format%3Djpg%26resize%3D1600%252C1050%26image_uri%3Dhttps%253A%252F%252Fs.yimg.com%252Fos%252Fcreatr-uploaded-images%252F2019-06%252Fdffe5260-8e13-11e9-b63f-ce5f582bb97a%26client%3Da1acac3e1b3290917d92%26signature%3De8a7567296fdb4cf66936f0c2064ac2f77b7c80d&client=amp-blogside-v2&signature=b0997fd1d0f76bb41ba86eca944320c591707ef4",
            "publishedAt": "2020-02-04T12:00:00Z",
            "content": "The company is initially offering two services: YubiEnterprise Subscription (which is available now), and YubiEnterprise Delivery (which is slated for release in spring). YubiEnterprise Subscription is an annual per-user pricing model that allows companies to… [+1265 chars]",
            "fullContentText": "The company is initially offering two services: YubiEnterprise Subscription (which is available now), and YubiEnterprise Delivery (which is slated for release in spring). YubiEnterprise Subscription is an annual per-user pricing model that allows companies to subscribe to a specific service tier based on how many users they need to support. The idea is that by moving away from a per-key purchasing model, organizations can better predict future spending and upgrades. This one is designed for businesses with 750 YubiKey users or more.\nThe second offering, YubiEnterprise Delivery, takes care of the actual logistics of key distribution. Via a dedicated Yubico portal (or software that can be integrated directly into existing IT software using public APIs) IT administrators can keep a track of who needs access to which keys, and the service will manage inventory, ship keys to the relevant people, monitor delivery and manage returns or replacements.\nYubico is one of the biggest players in the security key business -- and its tech is widespread across  Windows  and  iOS  devices -- so it's not a surprise that it's the first one to make this kind of offering. And it's much-needed -- businesses overall are gradually waking up to the benefits of security keys, but until now the logistical challenges in implementing them means they've largely been the sole preserve of smaller companies. Yubico's enterprise service opens the tech up to much larger companies, which will no doubt help accelerate its adoption overall.\nIn this article:  2FA  ,  business  ,  design  ,  enterprise  ,  gear  ,  hardware authentication  ,  personal computing  ,  personalcomputing  ,  security  ,  security key  ,  service  ,  Yubico  ,  Yubikey\n\nAll products recommended by Engadget are selected by our editorial team, independent of our parent company. Some of our stories include affiliate links. If you buy something through one of these links, we may earn an affiliate commission.\n\nComments\n\n47  Shares\n\nShare\n\nShare",
            "id": "k69plbdp"
        }
    ],
        "savedArticles": { },
    "entireCurrentArticleOF": {
        "source": {
            "id": null,
                "name": "Lifehacker.com"
        },
        "author": "Brendan Hesse",
            "title": "Here's How Much You'll Save With an Apple-Refurbished iPhone XS",
                "description": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS…",
                    "url": "https://lifehacker.com/heres-how-much-youll-save-with-an-apple-refurbished-iph-1841138323",
                        "urlToImage": "https://i.kinja-img.com/gawker-media/image/upload/c_fill,f_auto,fl_progressive,g_center,h_675,pg_1,q_80,w_1200/roo6mjgthvj1jtb5ayr1.jpg",
                            "publishedAt": "2020-01-21T21:00:00Z",
                                "content": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones through its online store and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS… [+2300 chars]",
                                    "fullContentText": "Apple now sells refurbished iPhone XS and iPhone XS Max smartphones  through its online store  and at certified retailers. This is good news for anyone who wants to upgrade from an iPhone 8 or older, but might not want to pay full price for an unlocked XS or XS Max (or or make the full jump to an iPhone 11). The XS and XS Max are barely two years old, and their tech is still plenty fast, so they’ll still be solid upgrades.\nThe real question, of course, is how much can you save on a refurbished iPhone XS or XS Max you buy directly from Apple. Exact final pricing depends on the model and configuration you go for, but all iPhone XS models appear to be $200 cheaper than buying new, while you’ll save around $250 for an iPhone XS Max. The highest prices you can expect to pay for the 512GB models of each phone (sans tax) are:\n-  512GB iPhone XS:  $999\n-  512GB iPhone XS Max:  $1,099\n\n(Note: all refurbished models sold by Apple are unlocked).\nFor reference, here’s a quick comparison of those same refurbished 512GB unlocked iPhone XS and XS Max models from popular third-party aftermarket sellers (these prices are based on the current stock at time of writing):\nSwappa\n-  iPhone XS:  $659\n-  iPhone XS Max:  $945\n\nBackmarket\n-  iPhone XS:  $799-$999\n-  iPhone XS Max:  $779\n\nGazelle\n-  iPhone XS:  $650\n-  iPhone XS Max:  $750\n\nRefurb.me\n-  iPhone XS:  $649-$999\n-  iPhone XS Max:  $749-$1099\n\nAs you can see, Apple’s prices are higher than other sellers, but buying from a third-party seller can be a gamble. Unless you’ve bought from a site or seller before, you can’t be sure the device runs as advertised. Some sellers offer warranties or trade-in programs should you wind up with a bad purchase, but it’s not a guarantee.\nConversely, all of Apple’s refurbished devices are  cleaned and tested by Apple  to ensure they meet the company’s quality standards. They’re not going to be as flawless as a brand-new device, but it can be pretty close. You’ll also get a one-year warranty on all devices and can extend coverage through AppleCare+.\nTo be clear, we’re not saying that you should only buy a refurbished iPhone XS or XS Max from Apple, but there are certain benefits Apple’s program guarantees its customers that can make buying one of their refurbished devices less risky, even with the higher prices. If you can find a better deal you trust elsewhere, then go for it. If not, Apple’s store is a decent option if you’re in the market for a slightly cheaper iPhone XS.",
                                        "id": "k69plbdg"
    },
    "currentTitleWordMap": [
        {
            "word": "Here's",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "How",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "Much",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "You'll",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "Save",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "With",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "an",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "Apple-Refurbished",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "iPhone",
            "isClicked": false,
            "isMouseOver": false
        },
        {
            "word": "XS",
            "isClicked": false,
            "isMouseOver": false
        }
    ],
        "currentAuthorWordMap": [
            {
                "word": "Brendan",
                "isClicked": false,
                "isMouseOver": false
            },
            {
                "word": "Hesse",
                "isClicked": false,
                "isMouseOver": false
            }
        ],
            "currentContentWordMap": [
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "now",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sells",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "smartphones",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "through",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "its",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "online",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "store",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "at",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "certified",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "retailers.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "This",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "good",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "news",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "anyone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "who",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "wants",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "upgrade",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "8",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "older,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "might",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "want",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pay",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "full",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "price",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "make",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "full",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "jump",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "11).",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "The",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "barely",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "two",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "years",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "old,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "their",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tech",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "still",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "plenty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "fast,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "so",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "they’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "still",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "solid",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "upgrades.\nThe",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "real",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "question,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "course,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "how",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "much",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buy",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "directly",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Exact",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "final",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pricing",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "depends",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "model",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "configuration",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "go",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "appear",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$200",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cheaper",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "than",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "new,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "while",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "save",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "around",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$250",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "an",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "The",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "highest",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "expect",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pay",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "each",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "phone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(sans",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tax)",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are:\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$1,099\n\n(Note:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sold",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "by",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked).\nFor",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "reference,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "here’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "quick",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "comparison",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "those",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "same",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "512GB",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "unlocked",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "models",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "popular",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "third-party",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "aftermarket",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "(these",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "based",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "current",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "stock",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "at",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "time",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "writing):\nSwappa\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$659\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$945\n\nBackmarket\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$799-$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$779\n\nGazelle\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$650\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$750\n\nRefurb.me\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$649-$999\n-",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max:",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "$749-$1099\n\nAs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "see,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "higher",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "than",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "other",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "third-party",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "seller",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "gamble.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Unless",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’ve",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "bought",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "site",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "seller",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "before,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can’t",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sure",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "device",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "runs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "advertised.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Some",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "sellers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "offer",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "warranties",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "trade-in",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "programs",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "should",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "wind",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "up",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "with",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "bad",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "purchase,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "guarantee.\nConversely,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cleaned",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "tested",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "by",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "ensure",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "they",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "meet",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "company’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "quality",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "standards.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "They’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "going",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "to",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "flawless",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "as",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "brand-new",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "device,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "pretty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "close.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "You’ll",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "also",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "get",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "one-year",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "warranty",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "on",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "all",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "and",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "extend",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "coverage",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "through",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "AppleCare+.\nTo",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "be",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "clear,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "we’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "saying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "that",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "should",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "only",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buy",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "or",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Max",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "from",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "but",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "there",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "are",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "certain",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "benefits",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "program",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "guarantees",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "its",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "customers",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "that",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "make",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "buying",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "one",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "of",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "their",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "refurbished",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "devices",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "less",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "risky,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "even",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "with",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "higher",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "prices.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "If",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "can",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "find",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "better",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "deal",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "trust",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "elsewhere,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "then",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "go",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "it.",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "If",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "not,",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "Apple’s",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "store",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "is",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "decent",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "option",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "if",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "you’re",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "in",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "the",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "market",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "for",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "a",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "slightly",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "cheaper",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "iPhone",
                    "isClicked": false,
                    "isMouseOver": false
                },
                {
                    "word": "XS.",
                    "isClicked": false,
                    "isMouseOver": false
                }
            ],
                "isInspiration": false,
                    "isPencilState": false,
                        "isMarkerState": false,
                            "isLoading": false,
                                "isDisplayFromSaved": false,
                                    "isPoetryFinished": false,
                                        "isOpen": false,
                                            "isNavOpen": false,
                                                "volNum": 1,
                                                    "isSmallStepper": ((window.innerWidth <= 425) ? true : false),
                                                        "step": "instructions",
                                                            "screenOrientation": "portrait",
                                                                "width": window.innerWidth,
                                                                    "height": window.innerHeight,
                                                                        "isExtraSmall": ((window.innerWidth <= 600) ? true : false),
                                                                            "isSmall": ((window.innerWidth > 600 && window.innerWidth <= 960) ? true : false),
                                                                                "isSNBP1": ((window.innerWidth > 425 && window.innerWidth <= 500) ? true : false),
                                                                                    "isSNBP2": ((window.innerWidth > 500 && window.innerWidth <= 760) ? true : false),
                                                                                        "isSNBP3": ((window.innerWidth > 760 && window.innerWidth <= 960) ? true : false)
}
